...
$Arr_sheet = array('PROGRESS SUMMARY','LR6 EACH GRADE','LR3 EACH GRADE','SUMMARY TROUBLE','TROUBLE','PALLET STATUS');
...
while ($s <= 5) {
    if ($Arr_sheet[$s] == "PROGRESS SUMMARY"){
    ...
    }elseif($Arr_sheet[$s] == "PALLET STATUS"){
        $objWorkSheet = $objPHPExcel->createSheet();

        $objPHPExcel->setActiveSheetIndex($s)
                    ->setCellValue('A1', 'PALLET STATUS '.$bln.' '.$not);

        $objPHPExcel->setActiveSheetIndex($s)->mergeCells('A1:I1');

        $objPHPExcel->setActiveSheetIndex($s)
                    ->setCellValue('A2','ID PRINT')
                    ->setCellValue('B2','ID PALLET')
                    ->setCellValue('C2','ASSY LINE')
                    ->setCellValue('D2','GRADE')
                    ->setCellValue('E2','PALLET NO.')
                    ->setCellValue('F2','PALLET QTY')
                    ->setCellValue('G2','POSITION')
                    ->setCellValue('H2','POSITION TIME')
                    ->setCellValue('I2','ID PALLET AGING');
        ...
        $objPHPExcel->getActiveSheet($s)->freezePane('A3');
        $sheet->getStyle('A2:I2')->getAlignment()->applyFromArray(array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,));
        ...
        while ($row5=oci_fetch_object($data5)) {
            $objPHPExcel->setActiveSheetIndex($s)
                        ...
                        ->setCellValue('C'.$no5, $row5->ASSY_LINE)
                        ->setCellValue('D'.$no5, $row5->CELL_TYPE)
                        ->setCellValue('E'.$no5, $row5->PALLET)
                        ->setCellValue('F'.$no5, $row5->QTY)
                        ->setCellValue('G'.$no5, $row5->REMARK)
                        ->setCellValue('H'.$no5, $row5->UPTO_DATE)
                        ->setCellValue('I'.$no5, $row5->ID_PALLET);

            if (is_null($row5->POSITION) OR $row5->POSITION == ''){
                $sheet->getStyle('A'.$no5.':I'.$no5)->applyFromArray(
                    array(
                        'fill' => array(
                            'type' => PHPExcel_Style_Fill::FILL_SOLID,
                            'color' => array('rgb' => 'FCE4D6')
            ...
            $objPHPExcel->getActiveSheet()->getStyle('F'.$no5)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
            $no5++;
        }

        // Rename worksheet
        $objPHPExcel->getActiveSheet()->setTitle('PALLET STATUS');
    }
    $s++;
}
