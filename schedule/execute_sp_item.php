...
echo date("Y-m-d H:i:s").' - START<br/>';
	...
	$ins = "insert into ztb_mrp_data_pck_delete (del_date, no_Id, description,
			...
			n_81, n_82, n_83, n_84, n_85, n_86, n_87, n_88, n_89, n_90, 
			ITEM_NO, ITEM_DESC
			from ztb_mrp_data_pck where item_no = '$item'";
	...
	// /*DELETE ztb_mrp_data*/
	// $del = "delete from ztb_mrp_data_pck where item_no = '$item'";
...
$arrNo++;
	...
	$msg = '';
	$sql = "BEGIN ZSP_MRP_PM_ITEM($item); END;";
	$stmt = oci_parse($connect, $sql);
	$res = oci_execute($stmt);
	$pesan = oci_error($stmt);
	$msg = $pesan['message'];
...
$arrNo++;
	...
	$msg = '';
	$sql = "BEGIN ZSP_UPDATE_MRP_ITEM($item); END;";
	$stmt = oci_parse($connect, $sql);
	$res = oci_execute($stmt);
	$pesan = oci_error($stmt);
	$msg = $pesan['message'];
