...
$cek = "select no_id, description, $value item_no, item_desc, $vBal balance
	from
	(select no_id, cc.description || ' ( ' || bb.this_inventory || ' )' description, $value item_no, item_desc,
	(select nvl(sum(bal_qty),0) as balance from po_details where item_no= aa.item_no and eta <= (select sysdate from dual)
	and bal_qty != 0) as balance
	from ztb_mrp_data_pck aa 
	left outer join whinventory bb on aa.item_no = bb.item_no 
	inner join item cc on aa.item_no = cc.item_no  
	$where
	order by cc.item_no, cc.description)" ;
