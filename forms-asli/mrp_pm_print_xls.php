...
if ($ck_week != "true"){
    ...
    $week = " a.item_no in (select distinct item_no from mps_header r
        ...
        where mps_date between (select sysdate from dual) and (select sysdate + $jumN from dual)
        ) and ";
}else{
    $week = "";
}
...
$sql = "
select * from (
      select distinct a.item_no upper_item_no, i.description as item_name, nvl(wh.this_inventory,0) as this_inventory, st.level_no, st.lower_item_no, st.line_no
      from mps_header a
      inner join (select distinct po_no, po_line_no, nvl(sum(mps_qty),0) as qty 
