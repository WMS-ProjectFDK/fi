...
<!DOCTYPE html>
	...
	*{
    ...
    <!-- EDIT -->
	<div id="dlg_e" class="easyui-dialog" style="width:1050px;" closed="true" data-options="position:'bottom'" buttons="#dlg-buttons_e">
	...
	<script type="text/javascript">
        ...
        function edit_item(){
            ...
            if (row){
            	var grp = row.ITEM_NO;
            	var grp_S = grp.substring(1,2);
            	$('#dlg').dialog('open').dialog('center').dialog('setTitle','Edit Parameter Budget ('+row.ITEM_NO+')');
            	$('#fm').form('load',row);
            	$('#ITEM_TYPE').combobox('setValue','ALKALINE');
            	$('#ITEM_ASSET').combobox('disabled','true');
            	$('#ITEM_GRP').combobox('setValue',grp_S);
