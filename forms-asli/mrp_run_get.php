<?php
	...
	$ins = "insert into ztb_mrp_data_delete (del_date, no_Id, description,
				...
				ITEM_NO, ITEM_DESC)
			select TO_DATE(TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS'), 'MM-DD-YYYY HH24:MI:SS') as del_date, no_Id, description, 
			n_1, n_2, n_3, n_4, n_5, n_6, n_7, n_8, n_9, n_10,
			n_11, n_12, n_13, n_14, n_15, n_16, n_17, n_18, n_19, n_20,
			n_21, n_22, n_23, n_24, n_25, n_26, n_27, n_28, n_29, n_30,
			n_31, n_32, n_33, n_34, n_35, n_36, n_37, n_38, n_39, n_40,
	...
	if($data_ins){
		/*DELETE ztb_mrp_data*/
		$del = "delete from ztb_mrp_data";
		$data_del = oci_parse($connect, $del);
		oci_execute($data_del);

		if($data_del){
			//RUN procedure
			$sql = "BEGIN ZSP_MRP_MATERIAL(); END;";
