	...
	*{
	...
	<div id="dlg" class="easyui-dialog" style="width:1050px;" closed="true" data-options="position:'bottom'" buttons="#dlg-buttons" data-options="modal:true">
        ...
        <fieldset style="width:1000px; margin:2px; padding:15px 15px;"><legend> LIST ITEM </legend>
	        <table id="dg_select_v" class="easyui-datagrid" toolbar="#toolbar_appr" style="width:100%;height:150px;" rownumbers="true" singleSelect="true"></table>
	        <div class="fitem"></div>
	        ...
	        <div id="toolbar_v">
	        	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" onclick="delete_vendor()">Delete Vendor</a>
	        </div>
        </fieldset>
        
    </div>
    <div id="dlg-buttons">
        <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="save_select_vendor()" style="width:90px">Save</a>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg').dialog('close')" style="width:90px">Cancel</a>
    </div>

	<div id="dlg_vendor" class="easyui-dialog" style="width:700px;height:250px;" closed="true" data-options="modal:true">
		<table id="dg_vendor" class="easyui-datagrid" toolbar='#toolbar_vendor' style="width:100%;height:100%;"></table>
	</div>
	<!-- END ADD VENDOR -->

	<!-- APPROVED -->
	<div id="dlg_appr" class="easyui-dialog" style="width:1050px;" closed="true" data-options="position:'bottom'" buttons="#dlg-buttons" data-options="modal:true">
        <fieldset style="width:1000px; margin:2px; padding:15px 15px;"><legend> QUOTATION HEADER </legend>
        	<div class="fitem">
	        	<span style="width:110px;display:inline-block;">QUOTATION NO.</span>
	        	<input style="width:180px;" name="QUOT_NO_appr" id="QUOT_NO_appr" class="easyui-textbox" disabled="" />
	        	<span style="width:35px;display:inline-block;"></span>
	        	<span style="width:120px;display:inline-block;">QUOTATION DATE</span>
	        	<input style="width:100px;" name="quot_date_appr" id="quot_date_appr" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser" disabled="" />
        	</div>
        </fieldset>
        <fieldset style="width:1000px; margin:2px; padding:15px 15px;"><legend> LIST ITEM </legend>
	        <table id="dg_appr" class="easyui-datagrid" toolbar="#toolbar_appr" style="width:100%;height:150px;" rownumbers="true" multieSelect="true"></table>
        </fieldset>
    ...
    <div id="dlg-buttons">
        ...
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg_appr').dialog('close')" style="width:90px">Cancel</a>
    </div>
    <!-- END APPROVED -->
	...
	<script type="text/javascript">
		...
		$('#dg_v').datagrid({
			columns:[[
				...
				{field:'COMPANY',title:'VENDOR', halign:'center', width:250},
		...
		$(function(){
			...
			$('#dg_appr').datagrid({
				columns:[[
					//{field:'ck_vendor_appr', checkbox: 'true'},
					{field:'ck_vendor_appr', width: 25, align:'center', title:'', halign: 'center',editor:{type:'checkbox',options:{on: 't',off: 'f'}}},
					/*{field: 'ck_vendor_appr', title: 'Approved', width: 50, align: 'center',
						editor: {
							type: 'checkbox',
							options: {
								on: 'Y',
					...
					{field:'QUOTATION_NO',title:'QUOTATION<br>NO.', halign:'center', width:120, hidden: true},
					{field:'ITEM_NO', halign:'center', width:120, hidden: true},
					{field:'ITEM_NO_TAMPIL',title:'ITEM<br>NO.', halign:'center', width:120},
					{field:'DESCRIPTION',title:'DESCRIPTION', halign:'center', width:300},
					{field:'SUPPLIER_CODE', hidden: true},
					{field:'COMPANY',title:'VENDOR', halign:'center', width:300},
					{field:'PRICE',title:'PRICE', halign:'center', align: 'right', width:100},
					{field: 'CURR',title:'CURRENCY', halign:'center', align:'center', width:100}
				]],
				onClickRow:function(rowIndex){
			    	$(this).datagrid('beginEdit', rowIndex);
			    }
		...
		function select_vendor(){
			...
			if (row){
				if (row.APPROVED=='1'){
					...
					$('#dlg').dialog('open').dialog('center').dialog('setTitle','Select Vendor & Price ('+row.QUOTATION_NO+')');
					$('#QUOT_NO').textbox('setValue',row.QUOTATION_NO);
					$('#quot_date').datebox('setValue',row.QUOTATION_DATE);
					$('#dg_v').datagrid('loadData',[]);
					$('#dg_select_v').datagrid({
						url: 'prf_quotes_detail.php?quotation='+row.QUOTATION_NO
					});
					$('#dg_v').datagrid({
						url: 'prf_quotes_detail_vendor.php?quotation='+row.QUOTATION_NO
					});
				}
			}else{
        ...
        function delete_vendor(){
        	var row = $('#dg_v').datagrid('getSelected');
        	if (row){
        		$.messager.confirm('Confirm','Are you sure you want to destroy this Item?',function(r){
	        		if (r){
	        			$.post('prf_quotes_Approve_destroy_vendor.php',{id:row.ITEM_NO, qt_no:row.QUOTATION_NO, vndor:row.SUPPLIER_CODE},function(result){
                            if (result.success){
                                $('#dg_v').datagrid('reload');
                            }else{
                                $.messager.show({title: 'Error',msg: result.errorMsg});
                            }
                        },'json');
	        			var index_v = $('#dg_v').datagrid('getRowIndex', row);
	        			$('#dg_v').datagrid('deleteRow',index_v);
	        		}
	        	})
        	}
        }

        function save_select_vendor(){	
        ...
        function quot_appr(){
			...
			if (row){
				if (row.APPROVED=='1'){
					...
					$.messager.alert('Information','Please insert vendor','info');	
				}else{
					$('#dlg_appr').dialog('open').dialog('center').dialog('setTitle','Approval ('+row.QUOTATION_NO+')');
					$('#QUOT_NO_appr').textbox('setValue',row.QUOTATION_NO);
					$('#quot_date_appr').datebox('setValue',row.QUOTATION_DATE);
					$('#dg_appr').datagrid({
						url: 'prf_quotes_detail_vendor.php?quotation='+row.QUOTATION_NO
					});
				}
        ...
        function save_quot_appr(){
			...
			if(ck_appr=='true'){
				$('#txt_notes').textbox('getValue');
				var t1 = $('#dg_appr').datagrid('getRows');
				var total1 = t1.length;
				for(i=0;i<total1;i++){
					$('#dg_appr').datagrid('endEdit',i);
					
					$.post('prf_quotes_approve_save2.php',{	
						qtno: $('#dg_appr').datagrid('getData').rows[i].QUOTATION_NO,	
						sts: $('#dg_appr').datagrid('getData').rows[i].ck_vendor_appr,
						brg: $('#dg_appr').datagrid('getData').rows[i].ITEM_NO,
						vnd: $('#dg_appr').datagrid('getData').rows[i].SUPPLIER_CODE,
						appr: ck_appr,
						note: $('#txt_notes').textbox('getValue')
					}).done(function(res){
						$('#dlg_appr').dialog('close');
						$('#dg').datagrid('reload');
					})
				}
				$.messager.alert('Information','Data Approved','info');
			}else{
