...
$sql_h = "select a.*, com.*, a.tterm as trade_term, cou.country, c.curr_short, c.curr_mark, 
	(select nvl(count(CARVED_STAMP),0) from po_details s where s.po_no = a.po_no) as date_code,
...
while ($data=oci_fetch_object($result)){
	...
	$content .= "
		<tr>
			".$noR."
			<td valign='middle' align='left' style='font-size:11px;height:25px;'>".$data->DESCRIPTION."<br/>[".$data->ITEM_NO."] - ".$data->ITEM."</td>
			".$t_r."
