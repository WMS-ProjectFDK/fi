	...
	*{
	...
	<div id="toolbar">
		...
		<div style="margin-top: 5px;margin: 5px;">
			...
			<a href="javascript:void(0)" style="width: 108px;" class="easyui-linkbutton c2" id="addInv" onclick="addInv()"><i class="fa fa-plus" aria-hidden="true"></i> Add Invoice</a>
			<a href="javascript:void(0)" style="width: 108px;" class="easyui-linkbutton c2" id="editInv" onclick="editInv()"><i class="fa fa-pencil" aria-hidden="true"></i> Edit Invoice</a>
			...
			<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:98%; float:left; margin:5px;"><legend>SI Sett</legend>
			  <div style="width:400px; height: 140px; float:left;">	
				...
				<div class="fitem">
					...
					<select style="width:120px;" name="ppbe_no" id="ppbe_no" class="easyui-combobox" 
		...
		<form id="f_edit" method="post" novalidate>
			<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:98%; float:left; margin:5px;">
				...
				<div class="fitem">
					...
					<span style="width:2px;display:inline-block;"></span>
					<span style="display:inline-block;">T. Terms</span>
					<input style="width:140px;" name="tterm_edit" id="tterm_edit" class="easyui-textbox" disabled=""/>
				</div>
			</fieldset>
			<div hidden="true" ><!-- hidden="true" -->
			...
			<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:98%; float:left; margin:5px;"><legend>SI Sett</legend>
			  <div style="width:400px; height: 140px; float:left;">	
				...
				<div class="fitem">
					...
					<!-- <input style="width:170px;" name="ppbe_no_edit" id="ppbe_no_edit" class="easyui-textbox"/> -->
				</div>
			  </div>
			  <div style="position:absolute; margin-left:410px; width:400px; height: 130px;">
			  	<div class="fitem">
	...
	<div>
		<!-- PRINT SETT -->
		<div id="dlg_print" class="easyui-dialog" style="width: 300px;height: 300px;" closed="true" data-options="modal:true">
		...
		$(function(){
			...
			$('#dg').datagrid({
			    ...
			    columns:[[
				    ...
				    {field:'CURR_MARK',title:'CURRENCY',width:50, halign: 'center', align: 'center',sortable:true},
				    ...
				    {field:'DELIVERY_UPDATE',title:'DELIVERY UPDATE', width:100, halign: 'center', align: 'center'},
			...
			var msg='';
				if(value == 'add'){
					...
					for(i2=0;i2<total;i2++){
						...
						if ($('#dg_add').datagrid('getData').rows[i2].U_PRICE == 0){
					...
					for(i2=0;i2<total;i2++){
						...
						if ($('#dg_edit').datagrid('getData').rows[i2].U_PRICE == 0){
							msg = $.messager.alert('INFORMATION','Price not Found','info');
							hasil=1;		
...
/*========================================== EDIT ===========================================*/
		function editInv(){
			...
			if(row){
			  if(row.STS == 0){
				...
				$('#sino_edit').combogrid({url: 'invoice_get_si_no.php?id='+row.CUSTOMER_CODE});
				...
				$('#dg_edit').datagrid({
			  	...
			  	$.messager.alert('INFORMATION',"Can't be Changed, delivery update is already",'info');
			  }
			}
		}
		...
		//	alert($('#notify_edit').textbox('getValue').replace(/\n/gi,"<br>"));
		//}
...
/*========================================== PRINT =========================================*/
		...
		function print(){
			...
			for(i=0;i<total;i++){
				var plan = $('#dg_print').datagrid('getData').rows[i].PLAN;
				var avlb = $('#dg_print').datagrid('getData').rows[i].AVAILABLE;
			}

			if(plan > avlb){
				if(document.getElementById('invoice').checked == true){
					s_type = 'invoice_proforma';
				}else if(document.getElementById('packing_list').checked == true){
					s_type = '';
			...
			if (s_type == ''){
				...
				if (s_type == 'final_si'){
					...
					$('#dlg_print').dialog('close');
					window.open('invoice_print_ppbe.php?si='+no_si+'&do='+no_do);
					...
					window.open('http://172.23.206.21/pglosas/entry/fl/fl_sheet3.asp?DO_NUM=1&do_no0='+no_do);
				}else{
					$('#dlg_print').dialog('close');
					window.open('http://sysfi01.indonesia.fdk.co.jp/pglosas/entry/iv2/iv2_sheet3_dummy.asp'+pdf_url);
					window.open('http://172.23.206.21/pglosas/entry/iv2/iv2_sheet3_dummy.asp'+pdf_url);
				}
			}
		}
					...
					$('#dg_part_SO').datagrid({
			            ...
			            onClickRow:function(id,row){
			            	...
			            	if (count>0) {
								...
								plt += parseFloat(row.PALLET);
								cart += parseFloat(row.CARTON);
