...
<!DOCTYPE html>
	...
	*{
	...
	<div id="dlg_print_rec" class="easyui-dialog" style="width: 1250px;height: 530px;" closed="true" buttons="#dlg-buttons-print-rec" data-options="modal:true">
		<fieldset style="border:1px solid #d0d0d0; border-radius:4px; width:1200px; height:430px; float:left;"><legend>Purchase Order Filter</legend>
			<div style="width:470px; float:left;">
				<div class="fitem">
				...
				<div class="fitem">
					...
					<a href="javascript:void(0)" style="width: 100px;" class="easyui-linkbutton c2" id="printPo" onclick="ViewPOData()"><i class="fa fa-filter" aria-hidden="true"></i> view PO</a>
			</div>
			<div class="fitem">
				<span style="width:80px;display:inline-block;">Supplier</span>
			...
			<div class="fitem">
				...
				<table id="dg_list_po"   style="border:1px solid #d0d0d0; border-radius:4px; width:1200px; height:310px; float:left;" class="easyui-datagrid" rownumbers="true" fitColumns="true" singleSelect="true">
	...
	<script type="text/javascript">
		...
		function ViewPOData (){
			...
			var item_no = document.getElementById('src_item').value;
			var po_no = $('#cmb_po_no_rec').combobox('getValue');
			var supplier_code = $('#supplier_rec').combobox('getValue');
			var date_rec;
			...
			alert(ck_supplier_rec);

			

			$('#dg_list_po').datagrid('load', {
				// date_awal: $('#date_a').datebox('getValue'),
				// date_akhir: $('#date_z').datebox('getValue'),
				// ck_date: ck_date_rec,
				// date_sts: date_rec,
				cmb_po: po_no,
				ck_po: ck_po_rec,
				supplier: supplier_code,
				ck_supplier: ck_supplier_rec,
				cmb_item_no: item_no,
				ck_item_no: ck_item_no_rec,
				endorder: ck_endorder_rec,
				src: ''
			});

			$('#dg_list_po').datagrid({
			url:'po_receive_view.php'
			});
		...
		$('#dg_list_po').datagrid({
				...
				sortOrder: 'asc',
			    columns:[[
				    {field:'PO_NO',title:'PO NO.',width:65,halign:'center', align:'center'},
	                {field:'LINE_NO',title:'LINE',width:30,halign:'center', align:'center'},
	                {field:'COMPANY',title:'SUPPLIER',width:100,halign:'center'},
	             	{field:'ITEM_NO',title:'ITEM NO',width:40,halign:'center',align:'center'},
	                {field:'DESCRIPTION',title:'DESCRIPTION',width:150,halign:'center',align:'right'},
	                {field:'QTY',title:'PO QTY',width:40,halign:'center',align:'right'},
	                {field:'GR_QTY',title:'TOTAL GR QTY',width:60,halign:'center', align:'center'},
	                {field:'BAL_QTY',title:'BAL QTY',width:65,halign:'center', align:'center'},
	                {field:'SLIP_NO',title:'GR NO',width:60,halign:'center',align:'right'},
	                {field:'SLIP_DATE',title:'GR DATE',width:65,halign:'center', align:'center'},
	                {field:'SLIP_QUANTITY',title:'GR QTY',width:65,halign:'center', align:'center'}  
			    ]]
		});  
