...
if (isset($_SESSION['id_wms'])) {
	...
	foreach($queries as $query){
		...
		if ($STS == 'NEW') {
			...
			$sql .= "  qty = $QTY_ORDER," ;
			$sql .= "  bal_qty = '$real_integer'," ;
			$sql .= "  data_date = sysdate," ;
			$sql .= "  duein_date = sysdate," ;
			$sql .= "  so_no = '$SO_NO'," ;
			$sql .= "  so_line_no = '$LINE_NO'," ;
			$sql .= "  operation_date = sysdate," ;
			$sql .= "  work_no        = '$WORK_ORDER'," ;
			$sql .= "  remark         = '$CR_REMARK'," ;
			$sql .= "  customer_po_no = '$PO_NO'," ;
			$sql .= "  cr_date        = '$CR_DATE'," ;
			$sql .= "  stuffy_date    = TO_DATE('$EX_FAC_DATE','yyyy-mm-dd')," ;
			$sql .= "  etd            = to_date('$ETD_DATE','yyyy-mm-dd')," ;
			$sql .= "  eta            = to_date('$ETA_DATE','yyyy-mm-dd')," ;
			$sql .= "  vessel         = '$VESSEL'," ;
			$sql .= "  crs_remark     = '$PPBE'," ;
			$sql .= "  customer_po_line_no = '$PO_LINE_NO'," ;
			$sql .= "  si_no          = '$SI_NO' " ;
			$sql .= "  where answer_no = '$STS' " ;
			// echo $sql;
			$data_upd = oci_parse($connect, $sql);
			oci_execute($data_upd);

			// INSERT ZTB_SHIPPING PLAN
			$sql2 =  "UPDATE ztb_shipping_plan SET ";
			$sql2 .= "ITEM_NO = '$ITEM_NO', ";
			$sql2 .= "ITEM_NAME = '$ITEM_NAME', ";
			$sql2 .= "CR_DATE = '$CR_DATE', ";
			$sql2 .= "EX_FACT = TO_DATE('$EX_FAC_DATE','yyyy-mm-dd'), ";
			$sql2 .= "ETD = TO_DATE('$ETD_DATE','yyyy-mm-dd'), ";
			$sql2 .= "ETA = TO_DATE('$ETA_DATE','yyyy-mm-dd'), ";
			$sql2 .= "SO_NO = '$SO_NO', ";
			$sql2 .= "PO_NO = '$PO_NO', ";
			$sql2 .= "LINE_NO = '$LINE_NO', ";
			$sql2 .= "QTY = $QTY_ORDER, ";
			$sql2 .= "VESSEL = '$VESSEL' ";
			$sql2 .= "WHERE INV_NO = '$PPBE' ";
			$sql2 .= "AND SI_NO = '$SI_NO' ";
			$sql2 .= "AND WO_NO = '$WORK_ORDER' ";
