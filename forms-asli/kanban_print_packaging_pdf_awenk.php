...
$plt = isset($_REQUEST['plt']) ? intval($_REQUEST['plt']) : '';
...
b.groups_pck, c.label_item_number, i.aging_day,
  ...
  rpad(cast(i.item as varchar(20)),20,' ') ||
  rpad(cast(a.wo_no as varchar(20)),20,' ') ||
  rpad(cast(a.plt_no as varchar(5)),5,' ') ||
  rpad(cast(a.plt_tot as varchar(5)),5,' ') ||
  ...
  lpad(cast(a.qty_prod as varchar(10)),10,'0') ||
  rpad(cast(a.type_item as varchar(10)),10,' ') ||
  rpad(cast(a.grade as varchar(5)),5,' ') || '       ' ||
  rpad(cast(nvl(itf.inner,' ') as varchar(14)),14,' ') ||'          ' ||
  rpad(cast(nvl(itf.blister,' ') as varchar(20)),20,' ') ||
  rpad(cast(nvl(itf.outer,' ') as varchar(14)),14,' ') 
as qr_code1,
  rpad(cast(a.wo_no as varchar(20)),20,' ') ||
  rpad(cast(a.item_no as varchar(10)),10,' ') ||
  rpad(nvl(cast(itf.berat_inner as varchar(8)),' ') ,8,' ') ||
  rpad(nvl(cast(itf.isi_inner as varchar(8)),' ') ,6,' ') ||
  rpad(nvl(cast(itf.inner as varchar(16)),' ') ,16,' ') ||
  rpad(nvl(cast(itf.toleransi_plus as varchar(8)),' ') ,10,' ') ||
  rpad(nvl(cast(itf.toleransi_minus as varchar(8)),' ') ,10,' ') ||
  rpad(cast(a.plt_no as varchar(4)),4,' ') ||
  case when itf.berat_inner < 1000 then 1 else 0 end || 
  case when itf.inner is null then 1 else 0 end
as qr_code2,
floor(c.operateion_time * qty_prod / (60 * man_power)) time_of_kanban,
...
where a.wo_no = '$wo' and a.plt_no = $plt
order by id asc";
...
while ($data=oci_fetch_object($result)){
	...
	if($plt_no_new != $plt_no_old){
	 ...
	 $link2 = 'http://172.23.225.85/wms/forms/qr_generate_timbangan.php?string='.$QR_CODE2;
