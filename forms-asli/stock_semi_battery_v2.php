...
$awal = date("F d, Y");
$_30hari = mktime(0,0,0,date("n"),date("j")+30,date("Y"));
$akhir = date("F d, Y", $_30hari);
if ($varConn=='Y'){ ?>
<!DOCTYPE html>
<html>
<head>
...
<body>
<h2 align='center'>STOCK SEMI BATTERY<br/><?php echo $awal;?>&nbsp;&nbsp; TO &nbsp;&nbsp;<?php echo $akhir;?></h2>

<table id="dg" class="easyui-datagrid" style="width:100%;height:960px;" url= "stock_semi_battery_v2_get.php" rownumbers="false"  fitColumns="true" 
	singleSelect="true" idField="TIPE">	
 	<thead>
		<tr>
			...
			<th rowspan="2" width="75" halign="center" align="center" field="AGING"><b>AGING</b></th>
			<th rowspan="2" width="70" halign="center" align="center" field="SAFETY_DAY"><b>SFTY<br/>DAY</b></th>
			<th rowspan="2" width="80" halign="center" align="center" field="GRADE"><b>GRADE</b></th>
			<th rowspan="2" width="100" halign="center" align="center" field="WORKING_DAY"><b>WORK<br/>DAY</b></th>
			...
			<th rowspan="2" width="120" halign="center" align="right" field="SUSPENDED" data-options="styler:cellStyler2"><b>SUSPEND</b></th>
		</tr>
	</thead>
	<thead>
		<tr>
...
<a href="stock_semi_battery_v2_execute.php" align="right" style="width: 100px;" class="easyui-linkbutton c2" onclick="RefreshData();"><i class="fa fa-refresh" aria-hidden="true"></i>Refresh Data</a>
...
<div id='dlg_info' class="easyui-dialog" style="width:800px;height:400px;padding:5px 5px;" closed="true" buttons="#dlg-buttons-view_prf" data-options="modal:true">
	...
	<a href="javascript:void(0)" id="printxls" style="width: 250px;" class="easyui-linkbutton c2" onclick="print_xls_info()"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Download To Excel</a>
</div>
...
<script type="text/javascript">
	function procedure_proses(){
		$.ajax({
			type: 'GET',
			url: 'stock_semi_battery_v2_execute.php'
		});
	}

	function table_proses(){
		$('#dg').datagrid({
			url: 'stock_semi_battery_v2_get.php'
		});
	}

	$(function(){
		var myData1 = setInterval(function(){procedure_proses()},15*60*1000);
		var myData2 = setInterval(function(){table_proses()},10*1000);
	...
	function info_heat(a,b){
		...
		$('#dg_info').datagrid({
			url: 'stock_semi_battery_v2_get_info.php?tipe='+a+'&jns='+b,
			singleSelect: true,
			rownumbers: true,
			fitColumns: true,
			showFooter: true,
	...
	function RefreshData(){
		alert('This process will takes 1-5 minutes please wait');
	}
	...
	function info_suspended(a,b){
		$('#dlg_info').dialog('open').dialog('setTitle','VIEW INFO SUSPEND (TIPE: '+a+')');
		$('#dg_info').datagrid({
			url: 'stock_semi_battery_v2_get_info_suspend.php?tipe='+a+'&jns='+b,
			singleSelect: true,
			rownumbers: true,
			fitColumns: true,
			showFooter: true,
		    columns:[[
			    ...
			    {field:'PALLET', title:'PALLET<br>NO.', width: 50, halign: 'center', align: 'center'},
			    {field:'ASSY_LINE', title:'ASSY LINE', width: 90, halign: 'center'},
			    {field:'CELL_TYPE', title:'CELL TYPE', width: 70, halign: 'center'},
			    {field:'SUSPEND_DATE', title:'SUSPEND DATE', width: 90, halign: 'center'},
			    {field:'PROBLEM', title:'PROBLEM', width: 200, halign: 'center'},
			    {field:'STATUS', title:'STATUS', width: 90, halign: 'center'},
	...
	function print_xls(){
		$('#dg').datagrid('toExcel','WIP_Info.xls')
	}

	function print_xls_info(){
		$('#dg_info').datagrid('toExcel','Info_detail.xls')
	}
</script>
