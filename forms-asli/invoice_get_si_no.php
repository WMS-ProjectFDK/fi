<?php
	...
	$sql = "select top 250 a.SI_NO, s.LOAD_PORT_CODE, s.LOAD_PORT,  substring(s.LOAD_PORT, 1, 30) as LOAD_PORT_F, s.person_name,
		a_sub.crs_remark PPBE, s.DISCH_PORT_CODE, s.DISCH_PORT, substring(s.DISCH_PORT, 1, 30) as DISCH_PORT_F, s.FINAL_DEST_CODE, 
		s.FINAL_DEST, substring(s.FINAL_DEST, 1, 30) as FINAL_DEST_F, s.CONSIGNEE_NAME, 
		s.CONSIGNEE_ADDR1, s.CONSIGNEE_ADDR2, s.CONSIGNEE_ADDR3, s.CONSIGNEE_ATTN, s.CONSIGNEE_TEL, s.GOODS_NAME, a.VESSEL, a.ETD as ETD_F, 
		a.ETA as ETA_F, f1.FORWARDER_CODE as FORWARDER_CODE, f2.FORWARDER_CODE as EMKL_CODE, c.METHOD_TYPE as SHIPPING_TYPE_CODE,
		...
		s.PAYMENT_TYPE, s.PAYMENT_REMARK, a.stuffy_date as ex_fact_date, a_sub.rmk as REMARK
		FROM (select a.SI_NO, min(a.ANSWER_NO) as ANSWER_NO_MIN, sum(a.QTY) as QTY_SUM, b.rmk, a.crs_remark 
				...
				where a.SI_NO is not null and a.CUSTOMER_CODE  = '996130' 
	...
	$result = oci_parse($connect, strtoupper($sql));
	$arrData = array();
	$arrNo = 0;
