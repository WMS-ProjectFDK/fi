...
$q= "select count(*) as jum from whinventory where this_month=to_char(sysdate,'yyyymm')" ;
$data_q = oci_parse($connect, $q);
oci_execute($data_q);
$dt_q = oci_fetch_object($data_q);
?>
	...
	*{
	...
	<div id="toolbar" style="padding:3px 3px;">
		...
		<div style="padding:5px 6px;">
    		...
    		<?php 
    		if ($dt_q->JUM <= 0){ ?>
    		<a href="javascript:void(0)" style="width: 150px;" class="easyui-linkbutton c2" disabled="true" onclick="add_gr()"><i class="fa fa-plus" aria-hidden="true"></i> Add Goods Receive</a>
    		<a href="javascript:void(0)" style="width: 150px;" class="easyui-linkbutton c2" disabled="true" onclick="edit_gr()"><i class="fa fa-pencil" aria-hidden="true"></i> Edit Goods Receive</a>
    		<a href="javascript:void(0)" style="width: 180px;" class="easyui-linkbutton c2" disabled="true" onclick="delete_gr()"><i class="fa fa-trash" aria-hidden="true"></i> Remove Goods Receive</a
    		>
    		<?php }else{ ?>
    		<a href="javascript:void(0)" style="width: 150px;" class="easyui-linkbutton c2" onclick="add_gr()"><i class="fa fa-plus" aria-hidden="true"></i> Add Goods Receive</a>
    		<a href="javascript:void(0)" style="width: 150px;" class="easyui-linkbutton c2" onclick="edit_gr()"><i class="fa fa-pencil" aria-hidden="true"></i> Edit Goods Receive</a>
    		<a href="javascript:void(0)" style="width: 180px;" class="easyui-linkbutton c2" onclick="delete_gr()"><i class="fa fa-trash" aria-hidden="true"></i> Remove Goods Receive</a>
    		<?php } ?>
    		<a href="javascript:void(0)" style="width: 180px;" class="easyui-linkbutton c2" onclick="print_gr()"><i class="fa fa-print" aria-hidden="true"></i> Print Goods Receive</a>
    	</div></div>
	</div>
	...
	<table id="dg" title="Goods Receive & Invoice Report" class="easyui-datagrid" toolbar="#toolbar	" style="width:100%;height:490px;" rownumbers="true" fitColumns="true" singleSelect="true"></table>
		...
		$(function(){
			...
			$('#dg').datagrid( {
			    columns:[[
	                ...
	                {field:'GR_DATE',title:'GOODS RECEIVE<br>DATE',width:70,halign:'center', align:'center'},
	                {field:'SUPPLIER_CODE',title:'SUPLIER<br>CODE',width:50,halign:'center', hidden: true},
	                {field:'COMPANY',title:'SUPLIER',width:240,halign:'center'},
	                ...
	                {field:'EX_RATE',title:'EXCHANGE<br>RATE',width:50,halign:'center', align:'center'},
	                {field:'AMT_L',title:'AMOUNT',width:80,halign:'center', align:'right'},
	                {field:'REMARK',title:'REMARK',width:80,halign:'center'},
	                {field:'PAYTERMS',title:'Payment<br>Terms',width:200,halign:'center'}
		...
		function simpan(){
			if($('#supp_no_add').textbox('getValue')==''){
				...
				for(i=0;i<total;i++){
					...
					if(i==total-1){
						//alert('HEADER - '+parseFloat(tot_amt_o).toFixed(2)+' - '+parseFloat(tot_amt_l).toFixed(2));
						$.post('gr_save.php',{
							gr_sts: 'HEADER',
							gr_no: $('#gr_no_add').textbox('getValue'),
							gr_line: jmrow,
			...
			$.ajax({
				...
				success: function(data){
					if(data[0].kode=='SUCCESS'){
						progress();
						simpan();
						/*START prosedeur buat gr_save*/
						$.ajax({
							type: 'GET',
							url: 'gr_save2.php'
						});
						/*END prosedeur buat gr_save*/
					}else{
...
//
		...
		function saveEdit(){
			...
			//DELETE GR
			var gr_Nya = $('#gr_no_edit').textbox('getValue');
			$.post('gr_destroy.php',{gr_no: gr_Nya},function(result){
				progress();
