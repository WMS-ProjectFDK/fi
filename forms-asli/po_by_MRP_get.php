<?php
	...
	if ($ck_prf_date != "true"){
		$date = "to_char(a.prf_date,'yyyy-mm-dd') between '$date_awal' and '$date_akhir' AND ";
	}else{
		$date = "";
	}
	...
	$where ="where $date $prf $item_no $supp a.customer_po_no='MRP' and b.remainder_qty !=0
		...
		and a.prf_no||'-'||b.line_no not in (select DISTINCT pod.prf_no||'-'|| pod.prf_line_no from po_details pod 
	...
	$sql = "select distinct a.prf_no, b.line_no as prf_line_no, a.prf_date, b.item_no, it.item, it.description, b.uom_q, u.unit, 
		b.qty, b.estimate_price, b.amt, to_char(b.require_date, 'YYYY-MM-DD') AS eta_date, b.remainder_qty,
		nvl(b.SUPPLIER_CODE,c.SUPPLIER_CODE) as SUPPLIER_CODE, s.COMPANY as SUPPLIER_NAME,
		nvl(b.CURR_CODE,c.CURR_CODE) as CURR_CODE, v.CURR_MARK, v.CURR_SHORT, to_char(b.CONFIRM_DATE, 'yyyy-mm-dd') as CONFIRM_DATE, 
		b.CONFIRM_PERSON_CODE, to_char(b.DELETE_DATE, 'yyyy-mm-dd') as DELETE_DATE, b.DELETE_PERSON_CODE, 
		...
		$where order by a.prf_no, a.prf_date";
