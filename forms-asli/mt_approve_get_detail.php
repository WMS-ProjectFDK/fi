<?php
	...
	while($row = oci_fetch_object($data)) {
		...
		$data_cek_rack = oci_parse($connect, $cek_rack);
		oci_execute($data_cek_rack);
		while($dt_rack = oci_fetch_object($data_cek_rack)) {
			$rack = $dt_rack->RACK;
			
			$cek_pallet = "select distinct pallet from ztb_wh_out_det where slip_no='$slip' and item_no='$item' and rack='$rack'";
			$data_cek_pallet = oci_parse($connect, $cek_pallet);
			oci_execute($data_cek_pallet);
			while ($dt_pallet = oci_fetch_object($data_cek_pallet)){
