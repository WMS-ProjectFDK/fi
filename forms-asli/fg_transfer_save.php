...
if (isset($_SESSION['id_wms'])){
	if($varConn == "Y"){
		...
		/*INI LIST WO TEMP YG SIAP DI UPDATE DENGAN WO ASLI*/
		$cek_PI = "select slip_no, slip_quantity from ztb_production_income 
			where wo_no = '$fgTrans_woTm' AND WO_NO_PI IS NULL
			...
			if($fgTrans_qtyA <> 0){
				if ($fgTrans_qtyA - $dt_cekPI->SLIP_QUANTITY > 0){
					...
					$upd = "update ztb_production_income set 
						wo_no_pi = '$fgTrans_wono',
						slip_date_pi = sysdate,
						slip_quantity_pi = slip_quantity
