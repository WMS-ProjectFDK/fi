...
<!DOCTYPE html>
	...
	*{
    ...
    <body  class="easyui-layout">
		<div id="layout" data-options="region: 'north',
					   iconCls: 'icon-save',
					   title: 'ASSEMBLY KANBAN SYSTEM',
					   collapsible: false,
					   modal: true,
					   tools: '#tt',
					   footer: '#footer'"
		...
		<!-- <div id="p" class="easyui-panel" title="ASSEMBLY KANBAN SYSTEM" data-options="tools:'#tt', footer:'#footer'"
			...
			<div id="toolbar_ng" style="width: 100%; float: left; padding: 10px;">
				...
				<div class="fitem">
					...
					<a href="javascript:void(0)" id="viewer" class="easyui-linkbutton c1" onclick="to_sparepart()" plain="true" style="width:320px; height:30px;font-weight: bold;"> SPAREPART ASSY MONITORING SYSTEM</a>
				</div>	
			</div>
		</div>
		<div id="tt">
		...
		<div id="toolbar_daily" style="padding: 5px 5px;">
			...
			<?php
			if($id_kanban == '9000160397'){ ?>
				<a href="javascript:void(0)" iconCls='icon-edit' class="easyui-linkbutton" onclick="edit_hasil()">Edit Hasil</a>
			<?php }else{ ?>
				<a href="javascript:void(0)" iconCls='icon-edit' class="easyui-linkbutton" onclick="edit_hasil()" disabled="true">Edit Hasil</a>
			<?php } ?>
		</div>
	    ...
	    <script type="text/javascript">	
	        ...
	        function scan(event){
		        ...
		        if(event.keyCode == 13 || event.which == 13){
					...
					if(split[7] == 'QC'){
						...
						$.ajax({
							type: 'GET',
							url: '../../forms/json/json_cek_start_kanban_1.php?id_print='+split[8],
			...
			function deleterow(target){
				...
				if (row){
					if(row.POSITION == 1){
						...
						$.messager.alert('INFORMATION','PALLET INI SUDAH DI AREA AGING, TIDAK BISA DI DELETE','info');
					}
				}
			}
			...
			function edit_hasil(){
	            ...
	            if (row){
	            	if(row.POSITION != 2){
		            	...
		            	$.messager.alert('INFORMATION','PALLET INI SUDAH DI AREA AGING, TIDAK BISA DI EDIT','info');
		            }
	            }
			}
		    ...
		    function to_sparepart(){
	    		var s = '?user=<?=$id_kanban?>';
	    		var url_spare = 'spare.php'+encodeURIComponent(s);
	    		window.open(decodeURIComponent(url_spare));
	    	}
		</script>
    </body>
    </html>
