...
$table_jum = "select count(*) as j from (select kanban_no, type_gel, type_zn, to_char(upto_date,'DD-MON-YYYY')||chr(13)||to_char(upto_date,'HH24:MI:SS') as upto_date, rowid 
		from ztb_assy_anode_gel_sts where flag = 0 and pemakaian_assy = 0 order by upto_date asc)";
...
<table class="w3-striped" align="center" style="width:650px;margin-top: 10px;">
	...
	<?php
		$table_list = "select kanban_no, type_gel, type_zn, to_char(upto_date,'DD-MON-YYYY')||chr(13)||to_char(upto_date,'HH24:MI:SS') as upto_date, rowid 
			from ztb_assy_anode_gel_sts where flag = 0 and pemakaian_assy = 0 order by upto_date asc";
