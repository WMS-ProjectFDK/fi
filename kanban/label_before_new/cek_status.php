<?php
	...
	if ($row_cek->JUM == 0){
		...
		if($row_cek->POS == 1){
			...
			$arrData[$arrNo] = array("kode"=>'BEFORE LABEL');
		}elseif($row_cek->STATUS == 2){
			$arrData[$arrNo] = array("kode"=>'BEFORE LABEL');
		}else{
			// cek status di ztb_lbl_trans jika 2 dan qty-qty_in_antrian > 0
			$cek = "select status, qty-qty_in_antrian as qty from ztb_lbl_trans where id_print = ".$id_print." and (remark = 'FINISH' OR remark = 'RESULT')";
			$cek_result = oci_parse($connect, $cek);
			oci_execute($cek_result);
			$row_cek = oci_fetch_object($cek_result);

			if($row_cek->STATUS == 2 and $row_cek->QTY > 0){
				$arrData[$arrNo] = array("kode"=>'BEFORE LABEL');
			}else{
