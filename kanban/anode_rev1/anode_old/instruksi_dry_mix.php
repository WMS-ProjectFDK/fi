		...
		<div style="width:auto;height:auto;padding:5px;">
			...
			<div align="center">
				...
				<div class="fitem" style="margin: 10px;">
					<!-- <input type="submit" style="width: 150px;" onclick="proses()" value="LAKUKAN DRY MIX"/> -->
					<a href="javascript:void(0)" style="width: 150px;" onclick="proses()" ><b>LAKUKAN DRY MIX</b></a>
				</div>
		...
		<?php
			if(isset($_POST['submit'])){
				...
				if($type_gel == '' AND $type_zn == '' AND $kanban_no == ''){
					...
					$upd_sts = "update ztb_assy_anode_gel_sts set flag=1, upto_date_drymix = to_date('".date('Y-m-d H:i:s')."','yyyy-mm-dd hh24:mi:ss')
						where rowid= (select b.rowid as row_sts from ztb_assy_anode_gel a 
	              					  ...
	              					  where b.flag=0 and id = $proses_no)";
					//echo $upd_sts.'<br/>';
					$data_upd_sts = oci_parse($connect, $upd_sts);
					oci_execute($data_upd_sts);

					if($data_upd_sts){
						echo "<script type='text/javascript'>alert('INSERT DATA SUCCESS');self.location.href = 'instruksi_dry_mix.php';</script>";
					}else{
