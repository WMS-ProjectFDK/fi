...
$sql = "select 
	ISNULL((select sum(qty) qty from assembly_qty_trans where replace(line, '#', '-') = '".$line."' and time between '$a1' and '$z1'),0) as shift_1,
	ISNULL((select sum(qty) qty from assembly_qty_trans where replace(line, '#', '-') = '".$line."' and time between '$a2' and '$z2'),0) as shift_2,
	ISNULL((select sum(qty) qty from assembly_qty_trans where replace(line, '#', '-') = '".$line."' and time between '$a3' and '$z3'),0) as shift_3,
	ISNULL((select sum(qty) qty from assembly_qty_trans where replace(line, '#', '-') = '".$line."' and time between '$at' and '$zt'),0) as total
";
