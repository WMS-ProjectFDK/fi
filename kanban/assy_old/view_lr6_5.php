...
$sql = "select * from
        ...
        where a.assy_line='LR06#5' and
        a.id = (select max(id) from ztb_assy_kanban where assy_line='LR06#5')
        ) aa
        left outer join 
        (
        select 'LR06#5' as line, 
        nvl(sum(qty_act_perpallet),0) as qty_today from ztb_assy_kanban
        where assy_line='LR06#5' AND to_char(tanggal_Actual,'yyyy-mm-dd')=(select to_char(sysdate,'yyyy-mm-dd') from dual)
        )bb on aa.assy_line = bb.line
        left outer join
        (
        select 'LR06#5' as line2, 
        coalesce(sum(qty_act_perpallet),0) as qty_monthly from ztb_assy_kanban
        where assy_line='LR06#5' AND to_char(tanggal_Actual,'yyyymm')=(select to_char(sysdate,'yyyymm') from dual)
