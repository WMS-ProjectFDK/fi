<?php
	...
	$rowno=0;
	$rs = "select cast(t.operation_date as varchar(10)) operation_date, t.section_code, sc.section, i.stock_subject_code, st.stock_subject, 
		  cast(t.slip_date as varchar(10))  slip_date, t.slip_type, sl.slip_name slip_name, t.slip_no, sl.in_out_flag,
			...
			case when i.STOCK_SUBJECT_CODE=5 then 
			  case when t.SLIP_TYPE=80 then t.WO_NO else ans.WORK_NO end 
			else NULL end  
			as WO_NO
			from [transaction] t
			...
			where i.delete_type  is null and t.section_code = '100' and t.item_no = '$id' and  t.accounting_month = '$month' 
