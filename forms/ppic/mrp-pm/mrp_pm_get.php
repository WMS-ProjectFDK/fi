...
if ($ck_week != "true"){
	...
	$week = " item_no in (select distinct item_no from mps_header r
		...
		where mps_date between cast(getdate() as date) and cast(getdate() + $jumN as date) and mps_qty > 0
		) and ";
}else{
	$jumN='';	$week='';
}
...
$cek = "
	select item_no,item_name,this_inventory,level_no from (
		select distinct a.item_no, i.description as item_name, isnull(wh.this_inventory,0) as this_inventory, st.level_no
		from mps_header a
		inner join (select distinct po_no, po_line_no, isnull(sum(mps_qty),0) as qty from ztb_mps_details group by po_no, po_line_no) b 
