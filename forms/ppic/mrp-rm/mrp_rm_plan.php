...
if($from == 'MRP'){
	...
	if ($cmb_item_no == ''){
		$sql = "select distinct b.uom_q, c.unit_pl, aa.item_no, line_no, bb.purchase_leadtime,item_type from ztb_mrp_data aa
		inner join item b on aa.item_no=b.item_no
		inner join unit c on b.uom_q = c.unit_code
		left outer join
		(select distinct max(line_no) line_no, item_no,purchase_leadtime from itemmaker group by item_no,purchase_leadtime)bb 
					on aa.item_no=bb.item_no
					where aa.item_type='".ltrim($cmb_type)."'";
			$flag = 0;
			$parm = $cmb_type;
	}else{
		$sql = "select distinct b.uom_q, c.unit_pl, aa.item_no, line_no, bb.purchase_leadtime,item_type from ztb_mrp_data aa
...
<div align="center" style="margin-top: 50px;">
	...
	<a class="easyui-linkbutton c2"  onclick="Runmrp()" style="width:200px;height: 30px;">RUN MRP FOR THIS ITEM</a>
...
<script type="text/javascript">
	...
	function Runmrp(){
		alert("Mohon di tunggu proses sedang berjalan.");
		$.post('mrp_run_get_item.php?item_no=<?php echo $item_no;?>').
		done(function(res){
				alert(res);
				location.reload();
