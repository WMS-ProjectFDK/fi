<?php
	...
	$sql = "select ISNULL(SUBSTRING(assy_line, 0, charindex( '#',assy_line)-1), 'assy_line') + '-' +cell_type as CELL_GRADE,sum(qty) QUANTITY, cast(upload_time as varchar(10)) as upload_time,bulan,tahun,tot ,case when tot=0 then 'N' else 'Y' end as ket
  ...
  group by bulan,tahun,upload_time,ISNULL(SUBSTRING(assy_line, 0, charindex( '#',assy_line)-1), 'assy_line') + '-' +cell_type,tot
  order by CELL_GRADE";
