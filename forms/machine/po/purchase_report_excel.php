...
if ($cmbR == 1){
  $sql = "select h.supplier_code, company.company Nama, 
    ...
    // select supplier_code, Nama, Stock_Subject,
    ...
    $sql = "select h.supplier_code, company.company Nama, 
		...
		left join currency c on h.curr_code = c.curr_code
		left outer join company on h.supplier_code = company.company_code 
		where h.gr_date between CAST('$dt_A' as DATE) AND CAST('$dt_Z' as DATE)
...
// $result_parts = sqlsrv_query($connect_spareparts, $sql_parts);
...
//START PARTS 
// while ($data_parts=sqlsrv_fetch_object($result_parts)){
//   if ($cmbR == 1){
