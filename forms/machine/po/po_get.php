<?php
	...
	if ($ck_item_no != "true"){
		$item_no = "a.po_no in (select po_no from sp_po_details where item_no = '$cmb_item_no') and ";
	}else{
		$item_no = "";
	}
	...
	if ($ck_eta != "true"){
		$eta = "a.po_no in (select po_no from sp_po_details where ETA = '$date_eta' and ";
	}else{
		$eta = "";
	}
	...
	$sql = "select top 150 a.PO_NO, convert(varchar, a.po_date, 23) as po_date, a.supplier_code, b.company, c.curr_short, c.curr_mark, a.amt_o, a.amt_l,
		a.remark1,a.req, prsn.person,replace(remark1,'CHAR(13)+CHAR(10)','\n') as remark1_2, cast(b.pdays as varchar(10))+'-'+b.pdesc as pterm, 
		cast(b.country_code as varchar(20))+'-'+cnt.country as country, a.curr_code, a.attn, a.shipto_code,a.tterm, 
		a.ex_rate, a.transport, replace(a.marks1,'CHAR(13)+CHAR(10)','\n') as marks1, a.di_output_type, a.revise, a.reason1
		from sp_po_header a
		left join sp_company b on a.supplier_code=b.company_code
	...
	while($row = sqlsrv_fetch_object($data)){
		...
		$rmk = $items[$rowno]->REMARK1_2;
		$items[$rowno]->REMARK1_2 = str_replace('\N','\n',$rmk);
