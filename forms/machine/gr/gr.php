...
$q= "select count(*) as jum from sp_whinventory where this_month=(SELECT CONVERT(nvarchar(6), getdate(), 112))" ;
...
<!DOCTYPE html>
    ...
    <body>
		...
		<!-- ADD GR -->
		<div id="dlg_add" class="easyui-dialog" style="width:1100px;height:450px;padding:5px 5px" closed="true" buttons="#dlg-buttons-add" data-options="modal:true, position: 'center'">
			<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:500px; float:left;height:80px;">
				<div class="fitem">
				...
				<div class="fitem">
					<span style="width:180px;display:inline-block;">Jenis Pembelian</span>
					<select style="width:55px;" name="jns_pembelian_add" id="jns_pembelian_add" class="easyui-combobox" >
						<option VALUE='AP' selected="">AP</option>
						<option VALUE='SIP'>SIP</option>
					</select>
				</div>
			</fieldset>
			<fieldset style="border:1px solid #d0d0d0; border-radius:4px; width:530px;margin-left: 510px;height:80px;">
				<div class="fitem">
					<span style="width:150px;display:inline-block;">Nomor Surat Jalan</span>
				</div>
				<div class="fitem">
		...
		<!-- EDIT GR -->
		<div id="dlg_edit" class="easyui-dialog" style="width:1100px;height:450px;padding:5px 5px" closed="true" buttons="#dlg-buttons-edit" data-options="modal:true, position: 'center'">
			<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:500px; float:left;height:80px;">
				<div class="fitem">
					<span style="width:80px;display:inline-block;">Supplier</span>
				...
				<div class="fitem">
					<span style="width:100px;display:inline-block;">Jenis Pembelian</span>
					<select style="width:200px;" name="jns_pembelian_edit" id="jns_pembelian_edit" class="easyui-combobox" >
						<option VALUE='AP' selected="">AP</option>
						<option VALUE='SIP'>SIP</option>
					</select>
				</div>
		...
		<table id="dg" title="GOODS RECEIVE SPAREPARTS" class="easyui-datagrid" toolbar="#toolbar	" style="width:100%;height:490px;" rownumbers="true" fitColumns="true" singleSelect="true"></table>
		...
		<script type="text/javascript">
			...
			$(function(){
				...
				$('#dg').datagrid( {
				    ...
				    columns:[[
		                ...
		                {field:'SJ_NO',title:'SURAT JALAN',width:80,halign:'center'},
		                ...
		                {field:'PURCHASE_TYPE',title:'PURCH.<br>TYPE',width:60,halign:'center'},
		                {field:'GR_TYPE',title:'GR<br>TYPE',width:150,halign:'center'},
		                {field:'BC_DOC',title:'BC Doc',width:80,halign:'center', hidden: true},
		                {field:'BC_NO',title:'BC No',width:80,halign:'center', hidden: true}
			...
			function download_report(){
				if (get_url != ''){
					...
					url_download = 'gr_download_xls.php'+get_url;//+get_url;
				...
				if($('#supp_no_add').textbox('getValue')==''){
					...
					for(i=0;i<total;i++){
						...
						dataRows.push({
							...
							gr_desc: $('#dg_add').datagrid('getData').rows[i].DESCRIPTION.replace(/[&\/\\#,+()$~%.'":*?<>{}]/g,'_'),
					...
					console.log('gr_save.php?data='+unescape(str_unescape));
					$.post('gr_save.php',{
			...
			function saveAdd(){
				...
				//console.log('json/json_kode_gr.php?gr='+grno+'&gr_date='+grdt+'&gr_sts=new');
				$.ajax({
					type: 'GET',
					url: 'json/json_kode_gr.php?gr='+grno+'&gr_date='+grdt+'&gr_sts=new',
					data: { kode:'kode' },
					success: function(data){
			...
			function simpan_edit(){
				...
				for(i=0;i<total;i++){
					...
					dataRows_Edit.push ({
						...
						gr_desc: $('#dg_edit').datagrid('getData').rows[i].DESCRIPTION.replace(/[&\/\\#,+()$~%.'":*?<>{}]/g,'_'),
	...
	//
			...
			function saveEdit(){
				...
				$('#cancel_edit').linkbutton('disable');
					//DELETE GR
				var grno = $('#gr_no_edit').textbox('getValue');
				var grdt = $('#gr_date_edit').datebox('getValue');
