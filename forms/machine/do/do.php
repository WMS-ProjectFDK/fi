...
<div id="toolbar" style="padding:3px 3px;">
	...
	<fieldset style="position:absolute;margin-left:495px;border-radius:4px;width: 500px;height: 100px;"><legend><span class="style3"><strong>Item Filter</strong></span></legend>
		<div class="fitem">
			...
			<select style="width:330px;" name="cmb_item_no" id="cmb_item_no" class="easyui-combobox" data-options=" mode:'remote',url:'json/json_item_all.php', method:'get', valueField:'id_item', textField:'id_name_item', panelHeight:'150px',
	...
	<fieldset style="margin-left: 1020px;border-radius:4px;height: 100px;"><legend><span class="style3"><strong>Print Select</strong></span></legend>
		<div class="fitem" align="center">
			<a id="print_laporan" href="javascript:void(0)" align='center' style="width: 180px;" class="easyui-linkbutton c2" onclick="print_report();" disabled=""><i class="fa fa-print" aria-hidden="true"></i> Report Issue Entry</a>
		</div>
	</fieldset>
...
<div id="dlg_add" class="easyui-dialog" style="width:1150px;height:400px;padding:5px 5px" closed="true" buttons="#dlg-buttons-add" data-options="modal:true">
	<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:1100px; float:left;height: 60px;">
		<div class="fitem">
			<span style="width:80px;display:inline-block;">SLIP NO</span>
			<input style="width:250px;" name="slip_no_add" id="slip_no_add" class="easyui-textbox" data-options="formatter:myformatter,parser:myparser" disabled=""/>
		...
		<div class="fitem">
			...
			<select style="width:350px;" name="cmb_company_add" id="cmb_company_add" class="easyui-combobox" data-options=" url:'json/json_company_plant.php', method:'get',valueField:'COMPANY_CODE', textField:'COMB_COMPANY', panelHeight:'150px;'" required=""></select>
		</div>
	</fieldset>
...
<table id="dg" title="SPAREPARTS TRANSACTION" class="easyui-datagrid" toolbar="#toolbar	" style="width:100%;height:490px;" rownumbers="true" fitColumns="true" singleSelect="true"></table>
	...
	$(function(){
		...
		$('#dg').datagrid( {
		    ...
		    columns:[[
                ...
                {field:'COST_PROCESS_NAME',title:'PROCESS NAME', halign:'center', width:100, sortable: true},
				{field:'CP_SECTION_NAME',title:'SECTION', halign:'center', width:100, sortable: true},
			...
			onExpandRow: function(index,row){
				...
				listbrg.datagrid({
					...
					columns:[[
		                ...
		                {field:'QTY', title:'QTY<br>ORDER', halign:'center', align:'right', width:70},
		                {field:'INVENTORY', title:'QTY<br>INVENTORY', halign:'center', align:'right', width:70},
		...
		$('#dg_add').datagrid({
		    ...
		    columns:[[
			    ...
			    {field:'STOCK', title:'STOCK', halign: 'center',width:75, align:'right'},
			    {field:'COST_SUBJECT_CODE', title:'COST PROCESS<br>CODE', width:350, halign: 'center', editor:{type:'combobox',
																											   options:{
																											   		url: 'json/json_cost_process_code.php',
																											   		valueField: 'COST_SUBJECT_CODE',
																											   		textField: 'COMB_SUBJECT_NAME'
																											   } 
																											  }
				},
			    {field:'SLIP_QTY', title:'SLIP<br>QTY', align:'right', halign: 'center', width:75, editor:{type:'numberbox',options:{precision:0,groupSeparator:','}}},
				{field:'REMARK', title:'REMARK', halign: 'center', width:100, align:'center', editor:{type:'textbox'}},
		...
		$('#dg_edit').datagrid({
		    ...
		    columns:[[
			    ...
			    {field:'DESCRIPTION', title:'DESCRIPTION', halign: 'center', width:250},
			    {field:'UNIT', title:'UoM', halign: 'center', width:65, align:'center'},
			    {field:'STOCK', title:'STOCK', halign: 'center',width:75, align:'right'},
			    {field:'COST_SUBJECT_CODE', title:'COST PROCESS<br>CODE', width:350, halign: 'center', editor:{type:'combobox',
																											   options:{
																											   		url: 'json/json_cost_process_code.php',
			    ...
			    {field:'QTY', title:'SLIP<br>QTY', align:'right', halign: 'center', width:75, editor:{type:'numberbox',options:{precision:0,groupSeparator:','}}},
			    {field:'REMARK', title:'REMARK', halign: 'center', width:100, align:'center', editor:{type:'textbox'}},
			    {field:'UOM_Q', hidden: true},
			    {field:'STOCK2', halign: 'center',width:120, align:'right', editor:{
    																			type:'numberbox',
...
var print_url='';
	...
	function filterData(){
		...
		print_url = '?date_awal='+$('#date_awal').datebox('getValue')+
					'&date_akhir='+$('#date_akhir').datebox('getValue')+
					'&ck_date='+ck_date+
					'&cmb_slip_no='+$('#cmb_slip_no').combobox('getValue')+
					'&ck_slip_no='+ck_slip_no+
		...
		$('#print_laporan').linkbutton('enable');
	...
	function add_mt(){
		$('#dlg_add').dialog('open').dialog('setTitle','Add Spareparts Transaction');
	...
	function find_brand_add(){
		...
		$('#dg_addBrand').datagrid({
			...
			columns:[[
                ...
                {field:'ITEM_NAME',title:'ITEM',width:200,halign:'center'},
                {field:'UNIT',title:'UNIT',width:45,halign:'center', align:'center'},
                {field:'BOM_LEVEL',title:'LEVEL',width:45,halign:'center', align:'center'},
                {field:'WORK_ORDER',title:'WO NO.',width:130,halign:'center'},
                {field:'DATE_CODE',title:'DATE CODE',width:70,halign:'center', align:'center'}
            ]],
            onDblClickRow:function(id,row){
			
            	n = row.ITEM_NO+'@'+row.ITEM_NAME+'@'+row.BOM_LEVEL+'@'+row.WORK_ORDER+'@'+row.DATE_CODE;
		
            	$('#brand_add').textbox('setValue', row.ITEM_NO+' - '+row.ITEM_NAME);
	...
	function add_do_add(){
		$('#dlg_addItem').dialog('open').dialog('setTitle','Search Spareparts');
	...
	function search_item_add(){
		...
		if(s_item != ''){
			console.log('do_getItem.php?item'+s_item+'&by='+s_by);
	...
	function simpan(){
		...
		if(slip_no_add == 'UNDEFINED' || slip_no_add == '' || slip_date_add == '' || cmb_slip_type_add =='' || cmb_company_add == ''){
			...
			for(i=0;i<total;i++){
				...
				dataRows.push({
					...
					do_subj: $('#dg_add').datagrid('getData').rows[i].COST_SUBJECT_CODE
				});
			}
			...
			console.log('do_save.php?data='+unescape(str_unescape));
			...
			$.post('do_save.php',{	
				...
				if(res == '"success"'){
					...
					$('#dg').datagrid('reload');
					$.messager.alert('INFORMATION','Insert Data Success..!!<br/>Slip No. : '+slip_no_add);
	...
	function saveAdd(){
		var d = new Date($('#slip_date_add').datebox('getValue'));
		var n = d.getMonth() + 1;
		var d1 = new Date();
		var n2 = d1.getMonth() +1;
		var t2 = d1.getDate();
		var flag = 0;
		...
		if (flag==1){
			$.messager.alert("WARNING", "Transaction is in difference month, please make sure your transaction in the correct month but If you still need transaction in difference month. please contact accounting or Warehouse.", "warning");
		}else{
			var url='';
			var slip = $('#cmb_slip_type_add').combobox('getValue');
			// console.log('json/json_kode_slip.php?slip='+slip);
			$.ajax({
				type: 'GET',
	...
	function edit_mt(){
		...
		if (row.STS == '0'){
			$('#dlg_edit').dialog('open').dialog('setTitle','Edit Transaction Spareparts ('+row.SLIP_NO+')');
			...
			$('#cmb_company_edit').combobox('setValue',row.COST_PROCESS_CODE);
	...
	function saveEdit(){
		var d_e = new Date($('#slip_date_edit').datebox('getValue'));
		var n_e = d_e.getMonth() + 1;
		var d1_e = new Date();
		var n2_e = d1_e.getMonth() + 1;
		var t2_e = d1_e.getDate();
		var flag_e = 0;

		console.log(t2_e+' - '+n2_e+' - '+n_e);

		if (n_e != n2_e){
			if (t2_e < 5 && (n2_e-1) == n_e){
				flag_e = 0;
			}else{
				flag_e = 1;
			}
			...
			if(slip_no_edit=='' && slip_date_edit=='' && cmb_slip_type_edit=='' && cmb_company_edit==''){
				...
				for(i=0;i<total;i++){
					...
					dataRows_Edit.push ({
						...
						do_subj: $('#dg_edit').datagrid('getData').rows[i].COST_SUBJECT_CODE
					});
				}
				...
				$.post('do_save_edit.php',{
					...
					if(res == '"success"'){
						...
						$('#dg').datagrid('reload');
						$.messager.alert('INFORMATION','Update Data Success..!!<br/>Slip No. : '+slip_no_edit,'info');
						$.messager.progress('close');
					}else{
						$.post('do_destroy.php',{id: slip_no_edit},'json');
	...
	function print_report(){
		console.log('do_report.php'+print_url);
		window.open('do_report.php'+print_url);	
	}
