...
$sql = "select SLIP_NO,cast(SLIP_DATE as varchar(10)) as SLIP_DATE,COMPANY_CODE,SLIP_TYPE,UPPER_ITEM_NO,UPPER_LEVEL_NO,UPPER_QTY,PERSON_CODE,DISPLAY_TYPE,APPROVAL_DATE,APPROVAL_PERSON_CODE,SECTION_CODE,WO_NO from sp_mte_header where slip_no='$do'";
$data_header = sqlsrv_query($connect, strtoupper($sql));
$dt = sqlsrv_fetch_object($data_header);


$qry = "select a.SLIP_NO,LINE_NO,a.ITEM_NO,QTY,u.unit,a.COST_PROCESS_CODE,DATE_CODE,REMARK,a.REG_DATE,a.UPTO_DATE,
	i.DESCRIPTION_ORG, RACK_ADDR, i.description_org as description, u.unit, co.cost_process_name
	from sp_mte_details a
	left join SP_MTE_HEADER b on a.SLIP_NO = b.SLIP_NO
	left join sp_item i on a.item_no=i.item_no
	left join sp_unit u on i.uom_q=u.unit_code
	left join SP_COSTPROCESS co on b.cost_process_code = co.cost_process_code
	where a.slip_no='$do' order by line_no asc";
...
$content .= "
	<thead>
		<tr>
			...
			<th valign='middle' align='center' style='font-size:10px;width:150px;height:25px;'>COST PROCCESS</th>
			<th valign='middle' align='center' style='font-size:10px;width:200px;height:25px;'>REMARK</th>
		</tr>
	</thead>";
...
while ($data=sqlsrv_fetch_object($result)){
	$content .= "
		<tr>
			...
			<td valign='middle' align='left' style='font-size:10px;height:15px;'>".$data->DESCRIPTION."</td>
			<td valign='middle' align='right' style='font-size:10px;height:15px;'>".number_format($data->QTY)."</td>
			<td valign='middle' align='center' style='font-size:10px;height:15px;'>".$data->UNIT."</td>
