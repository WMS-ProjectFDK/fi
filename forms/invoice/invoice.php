...
<!DOCTYPE html>
	...
	*{
			...
			<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:98%; float:left; margin:5px;"><legend>SI Sett</legend>
			  <div style="width:400px; height: 140px; float:left;">	
				...
				<div class="fitem">
					...
					<input style="width:50px;" name="load_port_kd_add" id="load_port_kd_add" class="easyui-textbox" disabled=""/>
					<input style="width:195px;" name="load_port_add" id="load_port_add" class="easyui-textbox" disabled=""/>
					<a href="javascript:void(0)" onclick="sett_port_load('add')">SET</a>
				</div>
				<div class="fitem">
					<span style="width:100px;display:inline-block;">DISCHARGE</span>
					<input style="width:50px;" name="discharge_kd_add" id="discharge_kd_add" class="easyui-textbox" disabled=""/>
					<input style="width:195px;" name="discharge_add" id="discharge_add" class="easyui-textbox" disabled=""/>
					<a href="javascript:void(0)" onclick="sett_port_disch('add')">SET</a>

				</div>
				<div class="fitem">
					<span style="width:100px;display:inline-block;">DESTINATION</span>
					<input style="width:50px;" name="destination_kd_add" id="destination_kd_add" class="easyui-textbox" disabled=""/>
					<input style="width:195px;" name="destination_add" id="destination_add" class="easyui-textbox" disabled=""/>
		...
		<form id="f_edit" method="post" novalidate>
			...
			<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:98%; float:left; margin:5px;"><legend>SI Sett</legend>
			  <div style="width:400px; height: 140px; float:left;">	
				...
				<div class="fitem">
					<span style="width:100px;display:inline-block;">LOADING PORT</span>
					<input style="width:50px;" name="load_port_kd_edit" id="load_port_kd_edit" class="easyui-textbox"/>
					<input style="width:190px;" name="load_port_edit" id="load_port_edit" class="easyui-textbox"/>
					<a href="javascript:void(0)" onclick="sett_port_load('edit')">SET</a>
				</div>
				<div class="fitem">
					<span style="width:100px;display:inline-block;">DISCHARGE</span>
					<input style="width:50px;" name="discharge_kd_edit" id="discharge_kd_edit" class="easyui-textbox"/>
					<input style="width:190px;" name="discharge_edit" id="discharge_edit" class="easyui-textbox"/>
					<a href="javascript:void(0)" onclick="sett_port_disch('edit')">SET</a>

				</div>
				<div class="fitem">
					<span style="width:100px;display:inline-block;">DESTINATION</span>
					<input style="width:50px;" name="destination_kd_edit" id="destination_kd_edit" class="easyui-textbox"/>
					<input style="width:190px;" name="destination_edit" id="destination_edit" class="easyui-textbox"/>
					<a href="javascript:void(0)" onclick="sett_port_dest('edit')">SET</a>
				</div>
				<div class="fitem">
					<span style="width:100px;display:inline-block;">PPBE No.</span>
					<input style="width:120px;" name="ppbe_no_edit" id="ppbe_no_edit" class="easyui-combobox" 
					data-options=" url:'../json/json_ppbe_no.php?user=<? echo $user_name; ?>',method:'get',valueField:'ppbe_no',textField:'ppbe_no', panelHeight:'100px'"/>
				</div>
			  </div>
			  <div style="position:absolute; margin-left:410px; width:400px; height: 130px;">
	...
	<div>
		<!-- PRINT SETT -->
		<div id="dlg_print" class="easyui-dialog" style="width: 330px;height: 380px;" closed="true" data-options="modal:true">
			...
			<div class="fitem" id="print_type">
				...
				<hr>
				<INPUT TYPE='radio' NAME='sheet_type' id='caution_mark' VALUE='caution_mark'/> CAUTION MARK<br/>
			</div>
		...
		<div id="dlg_transport" class="easyui-dialog" style="width: 450px;height: 150px;" closed="true" data-options="modal:true">
			...
			<div data-options="region:'south',border:false" style="text-align:right;padding:10px 0 0;">
				<a class="easyui-linkbutton c2" href="javascript:void(0)" onclick="save_sett_transport(state)" style="width:100px">
					<i class="fa fa-save" aria-hidden="true"></i> Save </a>
			</div>
		</div>
		<!-- END TRANSPORT -->
		...
		<!-- GW,NW,MSM SETT -->
		<div id="dlg_msm" class="easyui-dialog" style="width: 550px;height: auto;" closed="true" data-options="modal:true">
			<table id="dg_msm" class="easyui-datagrid" style="width:100%;height:auto;border-radius: 10px;" rownumbers="true"></table>
			<div data-options="region:'south',border:false" style="text-align:center;padding:5px 5px 5px 5px;">
				<a class="easyui-linkbutton c2" href="javascript:void(0)" onclick="save_msm()" style="width:100px">
					<i class="fa fa-print" aria-hidden="true"></i> save </a>
				<a class="easyui-linkbutton c2" href="javascript:void(0)" onclick="javascript:$('#dlg_msm').dialog('close');" style="width:100px">
					<i class="fa fa-ban" aria-hidden="true"></i> Cancel </a>
			</div>
		</div>
		<!-- END -->
		...
		function add_item_part(){
			...
			$('#dg_add').datagrid({
				...
				columns:[[
					...
					{field:'PALLET', hidden: true},
					{field:'CARTON', hidden: true},
					{field:'SHIPMENT', hidden: true},
					{field:'FORWARDER', hidden: true},
					{field:'ST', hidden: true},
					{field:'EMKL', hidden: true}
	            ]],
				onLoadSuccess: function (data) {
					var r = $('#dg_add').datagrid('getRows');
					for(i=0; i<r.length; i++){
						pallet += Math.ceil(r[i].PALLET);
						carton += Math.ceil(r[i].CARTON);
						shipment = r[i].SHIPMENT;
						EMKL = r[i].EMKL;
						FORWARDER = r[i].FORWARDER;
						ST = r[i].ST;
					}
					
					var remark = $('#remark_add').textbox('getValue');
					var rmk2 = "CARTON QTY = "+Math.ceil(carton)+" CARTONS\nPALLET QTY = "+Math.ceil(pallet)+" PALLETS";
					rmk_new = remark+'\n'+rmk2+'\n'+shipment+'\n' + 'PPBE : ' + $('#ppbe_no').combobox('getValue');
					st_value = 3;
					$('#remark_add').textbox('setValue',rmk_new);
					// $('#truck_add').combobox('setValue',EMKL);
					if(ST == 'OCEAN'){
						st_value = 2;
						state = 'add';
						$('#dlg_transport').dialog('open').dialog('setTitle','Setting Transport '+state );
		        		$('#dg_transport').datagrid('loadData',[]);
		        		$('#dg_transport').datagrid('beginEdit');
		        		$('#dg_transport').datagrid({
							rownumbers: true,
					...
					$('#trans_add').combobox('setText',ST);
					$('#trans_add').combobox('setValue',st_value);
					// $('#forwarder_add').combobox('setValue',FORWARDER);
				}
			});
		}
...
/*========================================== ADD ============================================*/
		...
		function addInv(){
			...
			$('#dg_add').datagrid({
				url: 'invoice_get_part_so.php?cust=123&si=1232',
				columns:[[
		...
		$('#sino_add').combogrid({
		    ...
		    onClickRow: function(rec){
		    	...
		    	if (r) {
			    	...
			    	$('#load_port_kd_add').textbox('setValue', r.LOAD_PORT_CODE);
			    	$('#load_port_add').textbox('setValue', r.LOAD_PORT);

			    	$('#discharge_kd_add').textbox('setValue', r.DISCH_PORT_CODE);
			    	$('#discharge_add').textbox('setValue', r.DISCH_PORT);

			    	$('#destination_kd_add').textbox('setValue', r.FINAL_DEST_CODE);
					...
					$('#ppbe_no').combobox('setValue',r.PPBE);
					$('#ppbe_no').combobox('enable');
					
					if (r.PAYMENT_TYPE == null){
						$('#remark_add').textbox('setValue','FREIGHT COLLECT');
					}else{
						var pty = r.PAYMENT_TYPE.toUpperCase();
						if (r.PAYMENT_REMARK == null){
							$('#remark_add').textbox('setValue','FREIGHT '+pty);	
						}else{
							var pre = r.PAYMENT_REMARK.toUpperCase();
							$('#remark_add').textbox('setValue','FREIGHT '+pty+' '+pre);	
						}
					}
		...
		function simpan(){
			...
			for(i=0;i<total;i++){
				...
				$.post('invoice_save.php',{
					...
					do_port_load_kd: $('#load_port_kd_add').textbox('getValue'),
					do_port_load: $('#load_port_add').textbox('getValue'),

					do_port_disg_kd: $('#discharge_kd_add').textbox('getValue'),
					do_port_disg: $('#discharge_add').textbox('getValue'),
					
					do_port_dest_kd: $('#destination_kd_add').textbox('getValue'),
					...
					//DETAILS
					do_line_no: jmrow,
					do_item_no: $('#dg_add').datagrid('getData').rows[i].ITEM_NO,
		...
		function editInv(){
			...
			if(row){
			  if(row.STS == 1){
				...
				$('#load_port_kd_edit').textbox('setValue', row.LOAD_PORT_CODE);
				$('#load_port_edit').textbox('setValue', row.LOAD_PORT);

				$('#discharge_kd_edit').textbox('setValue',row.DISCH_PORT_CODE);
				$('#discharge_edit').textbox('setValue',row.PORT_DISCHARGE);

				$('#destination_kd_edit').textbox('setValue',row.FINAL_DEST_CODE);
				$('#destination_edit').textbox('setValue',row.FINAL_DESTINATION);
		    ...
		    onClickRow: function(rec){
		    	...
		    	if (r) {
		    		/*if( parseInt(r.REMARK) != 1){*/
				    	...
				    	$('#load_port_kd_edit').textbox('setValue', r.LOAD_PORT_CODE);
				    	$('#load_port_edit').textbox('setValue', r.LOAD_PORT);
				    	
				    	$('#discharge_kd_edit').textbox('setValue', r.DISCH_PORT_CODE);
				    	$('#discharge_edit').textbox('setValue', r.DISCH_PORT);

				    	$('#destination_kd_edit').textbox('setValue', r.FINAL_DEST_CODE);
				    	$('#destination_edit').textbox('setValue', r.FINAL_DEST);
						...
						if (r.PAYMENT_TYPE == null){
							...
							var pty2 = r.PAYMENT_TYPE.toUpperCase();
							if (r.PAYMENT_REMARK == null){
								$('#remark_edit').textbox('setValue','FREIGHT '+pty2);	
							}else{
								var pre2 = r.PAYMENT_REMARK.toUpperCase();
								$('#remark_edit').textbox('setValue','FREIGHT '+pty2+' '+pre2);	
							}
						}
						...
						$('#ppbe_no_edit').combobox('setValue',r.PPBE);
		...
		function simpan_edit(){
			...
			for(i=0;i<total_e;i++){
				...
				$.post('invoice_save.php',{
					...
					do_port_load_kd: $('#load_port_kd_edit').textbox('getValue'),
					do_port_load: $('#load_port_edit').textbox('getValue'),
					
					do_port_disg_kd: $('#discharge_kd_edit').textbox('getValue'),
					do_port_disg: $('#discharge_edit').textbox('getValue'),
					
					do_port_dest_kd: $('#destination_kd_edit').textbox('getValue'),
					...
					//DETAILS
					do_line_no: jmrow_e,
					do_item_no: $('#dg_edit').datagrid('getData').rows[i].ITEM_NO,
...
/*========================================== PRINT =========================================*/
			...
			if(row){
				...
				$.get('inv_check.php',{
					...
					if(res == '"success"'){
						...
						if(res == 'Perhitungan MSM dan Gross ada yang berbeda, silahkan kalkulasi ulang. Jika masih terjadi silahkan hubungi IT.'){
							// alert('nah');
							$('#dg_msm').datagrid('loadData',[]);
							$('#dlg_msm').dialog('open').dialog('setTitle','GW, NW, MSM Setting ('+row.DO_NO+' ~ '+row.PPBE_NO+')');
							$('#dg_msm').datagrid({
							url: 'invoice_get_msm.php?id='+no_do+'&ppbe='+no_ppbe,
							fitColumns: true,
							singleSelect: true,
							columns:[[
				                {field:'STS',title:'DOCUMENT',width:100,halign:'center', align:'left'},
				                {field:'GW',title:'GROSS',width:75,halign:'center', align:'right', editor:{type:'numberbox',
																									   	   options:{precision:2,
																									   	   			groupSeparator:','
																									   	   		   }
																					   				  	  }
								},
				                {field:'NW',title:'NET',width:75,halign:'center', align:'right', editor:{type:'numberbox',
																									   	   options:{precision:2,
																									   	   			groupSeparator:','
																									   	   		   }
																					   				  	  }
								},
				                {field:'MSM',title:'MEASUREMENT',width:75,halign:'center', align:'right', editor:{type:'numberbox',
																									   	   options:{precision:3,
																									   	   			groupSeparator:','
																									   	   		   }
																					   				  	  }
		...
		function save_msm(){
			var dataRows = [];
			var t = $('#dg_msm').datagrid('getRows');
			var total = t.length;
			for(i=0;i<total;i++){
				$('#dg_msm').datagrid('endEdit',i);
				dataRows.push({
					s: $('#dg_msm').datagrid('getData').rows[i].STS,
					g: parseFloat($('#dg_msm').datagrid('getData').rows[i].GW.replace(/,/g,'')),
					n: parseFloat($('#dg_msm').datagrid('getData').rows[i].NW.replace(/,/g,'')),
					m: parseFloat($('#dg_msm').datagrid('getData').rows[i].MSM.replace(/,/g,'')),
					d: $('#dg_msm').datagrid('getData').rows[i].DOC
				});
			}
			...
			$.post('invoice_save_msm.php',{
				data: unescape(str_unescape)
			}).done(function(res){
				if(res == '"success"'){
					$('#dlg_msm').dialog('close');
					$('#dg').datagrid('reload');
		...
		function print(){
			...
			}else if(document.getElementById('caution_mark').checked == true){
				s_type = document.getElementById('caution_mark').value;
			}
			...
			if (s_type == ''){
				...
				if (s_type == 'final_si'){
					...
					window.open('../shipping_plan/invoice_print_si.php?si='+no_si+'&do='+no_do+'&si_sts=final_si');
					...
					$('#dlg_print').dialog('close');
					window.open('../shipping_plan/invoice_print_si.php?si='+no_si+'&do='+no_do+'&si_sts=si');
				}else if (s_type == 'ppbe'){
					$('#dlg_print').dialog('close');
		...
		function print_ppbeNya(){
			...
			if (jam == '' || mnt == ''){
				...
				window.open('../shipping_plan/invoice_print_ppbe.php?si='+no_si+'&do='+no_do+'&close_cargo='+close_c+'&jns_export='+s_exp+'&jam='+jam+'&menit='+mnt+'&note='+noteNya);
			}
		}
...
/*========================================== SETTING ========================================*/
			...
			$('#dg_contract').datagrid({
	            ...
	            onClickRow:function(id,row){
	            	...
	            	if(value=='add'){
						...
						$('#load_port_kd_add').textbox('setValue', rows.PORT_LOADING_CODE);
						$('#load_port_add').textbox('setValue', rows.LOADING_PORT);

				    	$('#discharge_kd_add').textbox('setValue', rows.PORT_DISCHARGE_CODE);
				    	$('#discharge_add').textbox('setValue', rows.DISCHARGE_PORT);

				    	$('#destination_kd_add').textbox('setValue', rows.FINAL_DESTINATION_CODE);
				    	$('#destination_add').textbox('setValue', rows.FINAL_DEST);
						...
						$('#load_port_kd_edit').textbox('setValue', rows.PORT_LOADING_CODE);
						$('#load_port_edit').textbox('setValue', rows.LOADING_PORT);

				    	$('#discharge_kd_edit').textbox('setValue', rows.PORT_DISCHARGE_CODE);
				    	$('#discharge_edit').textbox('setValue', rows.DISCHARGE_PORT);

				    	$('#destination_kd_edit').textbox('setValue', rows.FINAL_DESTINATION_CODE);
				    	$('#destination_edit').textbox('setValue', rows.FINAL_DEST);
		...
		function sett_port_load(value){
			...
			$('#dg_port').datagrid({
	            ...
	            onClickRow:function(id,row){
	            	...
	            	if(value=='add'){
	            		$('#load_port_kd_add').textbox('setValue',rows.PORT_CODE);
	            		$('#load_port_add').textbox('setValue',rows.PORT);
	            	}else if (value == 'edit'){
	            		$('#load_port_kd_edit').textbox('setValue',rows.PORT_CODE);
	            		$('#load_port_edit').textbox('setValue',rows.PORT);
	            	}
		...
		function sett_port_disch(value){
			...
			$('#dg_port').datagrid({
	            ...
	            onClickRow:function(id,row){
	            	var rows = $('#dg_port').datagrid('getSelected');
	            	if(value=='add'){
	            		$('#discharge_kd_add').textbox('setValue',rows.PORT_CODE);
	            		$('#discharge_add').textbox('setValue',rows.PORT);
	            	}else if(value == 'edit'){
	            		$('#discharge_kd_edit').textbox('setValue',rows.PORT_CODE);
	            		$('#discharge_edit').textbox('setValue',rows.PORT);
	            	}
	            	$('#dlg_port').dialog('close');
				}
			});
		...
		function sett_port_dest(value){
			...
			$('#dg_port').datagrid({
	            ...
	            onClickRow:function(id,row){
	            	var rows = $('#dg_port').datagrid('getSelected');
	            	if(value=='add'){
		            	$('#destination_kd_add').textbox('setValue',rows.PORT_CODE);
		            	$('#destination_add').textbox('setValue',rows.PORT);
		            }else if(value == 'edit'){
		            	$('#destination_kd_edit').textbox('setValue',rows.PORT_CODE);
		            	$('#destination_edit').textbox('setValue',rows.PORT);
		            }
	            	$('#dlg_port').dialog('close');
				}
			});
		...
		function save_sett_transport(value){
			...
			for(i=0;i<total;i++){
				...
				if (q !== 0){
					...
					if(value=='add'){
						...
						var rmk_ship = $('#remark_edit').textbox('getValue');
					}
					...
					if (m != 0 || s != 0 || q != 0){
						...
						if(value=='add'){
							...
							$('#remark_edit').textbox('setValue',rmk_new_ship);
						}
			...
			if(value=='add'){
				...
				$('#transport1_edit').textbox('setValue',ids[0]);
