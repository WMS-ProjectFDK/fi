...
if (isset($_SESSION['id_wms'])){
	...
	echo $jum;
	if($jum > 0){
		$upd = "update ztb_user_access set access_add='$a', access_update='$e', access_delete='$d', access_view='$v', access_print='$p', last_update=getdate(), update_by='$userentry' where person_code='$user_id' and menu_id=$menu_id ";
		$result = sqlsrv_query($connect, $upd);
	}else{
		$ins = "insert into ztb_user_access VALUES ('$user_id',$menu_id,'$a','$e','$d','$v','$p',getdate(),'$userentry','T')";
		$result = sqlsrv_query($connect, $ins);
	}
