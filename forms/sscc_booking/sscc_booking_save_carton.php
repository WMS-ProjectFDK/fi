...
if (isset($_SESSION['id_wms'])){
	...
	foreach($queries as $query){
		...
		$sql = "select wo,z.item, start_carton+(($crt_plt-1)*pallet_ctn_number) as StartCarton, 
			total_carton, ceiling(quantity/external_unit_number/pi.pallet_ctn_number) as TotalPallet,
		...
		while ($row = sqlsrv_fetch_object($dataNya)) {
			...
			for ($i=$FromCarton; $i<$toCarton ; $i++) {
				...
				if($i<$finish){
					...
					$ins = "insert into ztb_sscc ($crt_field) select $crt_value";
					// echo $ins;
