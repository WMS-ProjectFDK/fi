...
$query = "select left(cast(lower_item_no as varchar(10))+'          ',10) +
    left(cast(date_code as varchar(10))+'          ',10) +
    right('                            '+cast(wo_no as varchar(28)),28) +
    left(cast(item_no as varchar(10))+'          ',10) as qr_code
    from (
    select distinct ID, wo_no, date_code, lower_item_no, b.item_no,
    CEILING((round((quantity * qty_prod) / quantity_base,0) + CEILING((quantity * qty_prod) / quantity_base  * failure_rate/100)) / isnull(bundle_qty,1)) * isnull(bundle_qty,1) as Qty,
    ...
    from structure a 
    inner join (Select ID,WO_No ,ztb_plan_M.Item_No,Qty_Prod,date_prod,date_code from ztb_plan_M ) b on a.upper_item_no = b.item_no 
