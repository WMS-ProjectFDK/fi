...
ELSE
	IF REQUEST.FORM("type") = "NEW" THEN
		IF IsNumeric(Request.Form("item_no")) THEN
			SQL_item = "SELECT * FROM ITEM WHERE ITEM_NO = " & CLng(Request.Form("item_no"))
		...
		If (Session("NGM") = "1") Then
			If CStr(REQUEST("ngmc")) = "on" Then 
				If Request.Form("item_no") > 9999999 Then
					ITEM_NO_FLG = 1
					ERROR_MESSAGE = ERROR_MESSAGE & "Automatic registration cannot be performed (Item no is 8 digit).<Br>"
				Else
					If IsNumeric(Request.Form("item_no")) THEN
						SQL_item = "SELECT * FROM ITEM WHERE ITEM_NO = " & CLng(Request.Form("item_no")) + 90000000
						SET Rec_item = DB.Execute(SQL_item)
						IF Rec_item.eof or Rec_item.bof THEN
						Else
							ITEM_NO_FLG = 1
							ERROR_MESSAGE = ERROR_MESSAGE & "Automatic registration cannot be performed (Same Item no : " & CLng(Request.Form("item_no")) + 90000000 & "). <Br>"
						End If
					End If
				End If
			End If
			If CStr(REQUEST("no_type")) = "NGM" Then
				If Request.Form("item_no") < 90000000 Then
					ITEM_NO_FLG = 1
					ERROR_MESSAGE = ERROR_MESSAGE & "Please input a item no to register itself with '9XXXXXXXX' (Materials 2).<Br>"
				End If
			End If
		End If
	End If
...
' NEXT TERM PRICE
'************************************
 If  Request.Form("next_term_price") = "" or  Request.Form("next_term_price") = "0" Then
      Next_term_price = Request("Standard_price")
 Else
      Next_term_price = Request("Next_term_price")
 End IF
 IF next_term_price <> "" THEN
    IF IsNumeric(next_term_price) THEN
        CALL DECIMAL_CHECK(next_term_price, 4, RESULT)
        IF RESULT = "0" THEN
             NEXT_TERM_PRICE_FLG = 0
        ELSE
...
IF Request.Form("weight") <> "" THEN
	IF IsNumeric(Request.Form("weight")) THEN
		...
		IF RESULT = "0" THEN
			If REQUEST.FORM("weight") > 999999 Then
				WEIGHT_FLG = 1
...
<% If (Session("NGM") = "1") Then %> <Input Type="hidden" Name="ngmc" Value="<%= request.form("ngmc") %>"> <% End If %>
<Input Type="hidden" Name="type" Value="<%= request.form("type") %>">
<Input Type="hidden" Name="item_no" Value="<%= Request.form("item_no") %>" >
<Input Type="hidden" Name="item_code" Value="<%= Request.form("item_code") %>" >
<Input Type="hidden" Name="item" Value="<%= UCase(Request.form("item")) %>" >
...
<Input Type="hidden" Name="next_term_price" Value="<%= next_term_price%>" >
...
<Input Type="hidden" Name="drawing_no" Value="<%= UCase(REQUEST.FORM("drawing_no")) %>">
<Input Type="hidden" Name="drawing_rev" Value="<%= UCase(REQUEST.FORM("drawing_rev")) %>">
<Input Type="hidden" Name="applicable_model" Value="<%= UCase(REQUEST.FORM("applicable_model")) %>">
<Input Type="hidden" Name="catalog_no" Value="<%= UCase(REQUEST.FORM("catalog_no")) %>">
...
<% IF ITEM_NO_FLG = 1 THEN %>
 <Td ColSpan="2" BgColor="#FF0000"><%= Request.form("item_no") %>&nbsp;
<% ELSE %>
 <Td ColSpan="2"><%= Request.form("item_no") %>
<% END IF %>
<% If (Session("NGM") = "1") Then %>
  <% If CStr(REQUEST("no_type")) = "NGM" THEN %>
	Registration of a NG Materials
  <% Else %>
	Make an NG Materials <% If CStr(REQUEST("ngmc")) = "on" Then %> <B>Yes</B> <% Else %> <B>No</B> <% End If %>
  <% End If %>
<% End If %>
</Td>
...
<% IF NEXT_TERM_PRICE_FLG = 1 THEN %>
 ...
 <Td BgColor="#ff0000"><%= next_term_price %>&nbsp;</Td>
<% ELSE %>
 <!--<Td ><%= Request.form("next_term_price") %>&nbsp;</Td>-->
 <Td ><%=next_term_price %>&nbsp;</Td>
<% END IF %>
