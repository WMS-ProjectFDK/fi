...
#     $sql .= " where i.delete_type is null ";
     $sql .= " where ( i.delete_type is null  or i.delete_type is not null)";
     if($word ne ""){
     $sql .= " and (    1=0 " ;
...
print <<ENDOFTEXT;
        	<tr BGCOLOR=White>
        	<td ALIGN=Right nowrap>$ITEM_NO</td>
        	<td nowrap>$ITEM</td>
        	<td nowrap>$DESCRIPTION<BR></td>
        	<td nowrap>$MAK<BR></td>
        	<td nowrap>$COUNTRY</td>
        	<td nowrap>$CLASS</td>
ENDOFTEXT
if($DELETE_TYPE eq ""){
       print "<td nowrap><A HREF=list_item3.cgi?KEYWORD=$KEYWORD&item_no=$ITEM_NO>DETAIL</A></td>" ;
}else{
       print "<td nowrap><Font Color='red'><B> While processing the Delete</B></Font> </td>" ;
}
       print "</tr>" ;
