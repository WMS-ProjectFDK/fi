...
<!DOCTYPE html>
	...
	*{
		...
		<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:1045px; float:left; margin:5px;"><legend>Create PO</legend>
			<div style="width:395px; height: 80px; float:left;">	
				...
				<div class="fitem">
					...
					<input style="width:163px;" name="po_date_add" id="po_date_add" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser" value="<?date();?>"/>
			...
			<div>
				<div class="fitem">
					...
					<span style="width: 310px;display:inline-block;">Remark</span>
	  ...
	  <form id="ff_edit" method="post" novalidate>	
		...
		<fieldset style="border:1px solid #d0d0d0; border-radius:2px; width:1045px; float:left; margin:5px;"><legend>Create PO</legend>
			<div style="width:395px; height: 80px; float:left;">	
				...
				<div class="fitem">
					<span style="width:50px;display:inline-block;">PO No.</span>
					<input style="width:163px;" name="po_no_edit" id="po_no_edit" class="easyui-textbox" required="" maxlength="15"/>
					<span style="width:5px;display:inline-block;"></span>
					
				</div>
				<div class="fitem">
					<span style="width:55px;display:inline-block;">PO Date</span>
					<input style="width:163px;" name="po_date_edit" id="po_date_edit" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser" value="<?date();?>"/>
					<span style="display:inline-block;"></span>
					<span style="width:60px;display:inline-block;">Transport</span>
					<select style="width:85px;" name="trans_edit" id="trans_edit" class="easyui-combobox" data-options="panelHeight:'70px'" required="">
						<option selected="" value=""></option>
						<option value="1">AIR</option>
						<option value="2">OCEAN</option>
						<option value="3">TRUCK</option>
			...
			<div>
				...
				<div class="fitem">
					<input style="width: 310px; height: 56px;" name="remark_edit" id="remark_edit"  multiline="true" class="easyui-textbox" data-options="validType:'length[0,999]'"/>
					<input style="width: 310px; height: 56px;" name="shipp_mark_edit" id="shipp_mark_edit"  multiline="true" class="easyui-textbox" data-options="validType:'length[0,299]'"/>
				</div>
			</div>
		</fieldset>
		<div style="clear:both;margin-bottom:10px;"></div>
	...
	<div id="toolbar">
		...
		<fieldset style="border:1px solid #d0d0d0; border-radius:4px; height:100px; margin-left:965px;"><legend>Purchase Order Report</legend>
		  <div align="center">
			   ...
			   <a href="javascript:void(0)" class="easyui-linkbutton c2" onclick="print_po_sts_xls()" iconCls="icon-excel" plain="true" style="width:140px"> Print Report PO</a>
		  </div>
	...
	<table id="dg" title="PURCHASE ORDER SPAREPARTS" toolbar="#toolbar" class="easyui-datagrid" style="width:100%;height:490px;"></table>
	...
	<script type="text/javascript">
		...
		function addPo(){
			...
			$('#dg_add').datagrid({
			    ...
			    columns:[[
				    ...
				    {field:'ITEM', hidden: true},//title:'ITEM NAME', width:100, halign: 'center'},//, hidden: true},
				    ...
				    {field:'MPR_NO', title:'MPR NO.', halign: 'center', width:100, align:'center', editor:{
				    																					type:'textbox'
				    																				}
				    },
				    {field:'PRF_NOMOR', hidden:true},// title:'PRF NO', halign: 'center', width:100, align:'center'},
				    {field:'PRF_NO', hidden: true},
				    {field:'PRF_LINE_NO', hidden:true},//title:'PRF<br/>LINE NO', halign: 'center', width:50, align:'center'},
		...
		function simpan(){
			...
			for(i=0;i<total;i++){
				...
				dataRows.push({
					...
					po_dt_code: $('#dg_add').datagrid('getData').rows[i].MPR_NO,
			...
			console.log('po_save.php?data='+unescape(str_unescape));
		...
		function editPo(){
			...
			if (row){
				...
				$('#remark_edit').textbox('setValue',row.REMARK1_2);
				...
				$('#dg_edit').datagrid({
				    ...
				    columns:[[
					    ...
					    {field:'CARVED_STAMP', title:'MPR NO.', halign: 'center', width:100, align:'center', editor:{
					    																					type:'textbox'
					    																				}
					    },
					    {field:'PRF_NOMOR', title:'PRF NO', halign: 'center', width:100, align:'center', hidden: true},
					    {field:'PRF_NO', title:'PRF NO', hidden: true},// 
					    {field:'PRF_LINE_NO', title:'PRF<br/>LINE NO', halign: 'center', width:50, align:'center', hidden: true},
					    {field:'ORIGIN_CODE', hidden:true},
					    {field:'UOM_Q', hidden:true}
				...
				$.messager.confirm('Confirm','Are you sure you want to remove?',function(r){
					if(r){
						if(row.LINE_NO == 'NEW'){
							...
							console.log('po_destroy_dtl.php?po='+$('#po_no_edit').textbox('getValue')+'&item='+row.ITEM_NO+'&line='+row.LINE_NO);
