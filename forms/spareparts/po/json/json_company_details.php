<?php
	...
	$sql = "select poh.supplier_code,coalesce(poh.di_output_type,0) di_output_type,poh.section_code,poh.fdk_person_code,poh.attn,poh.cc,
		poh.po_no,convert(varchar, poh.po_date, 23) as po_date,poh.curr_code,poh.req requestor,poh.ex_rate,
		poh.tterm,poh.pby,coalesce(com.pdays, poh.pdays) pdays,coalesce(com.pdesc, poh.pdesc) pdesc, poh.revise,poh.date_type,
		poh.shipto_code,replace(poh.remark1,'CHAR(13)+CHAR(10)','') remark1,replace(poh.marks1,'CHAR(13)+CHAR(10)','') shipping_mark,replace(poh.reason1,'char(13)','') reason,
		poh.amt_o total_amount,poh.transport, com.company_code, com.company, com.country_code, cnt.country
		from sp_po_header poh
        left join sp_company com on poh.supplier_code=com.company_code
        left join country cnt on com.country_code=cnt.country_code
		where poh.supplier_code = $id and 
		poh.po_no =(select max(po_no) from sp_po_header where supplier_code=poh.supplier_code and po_date=(select max(po_date) from sp_po_header 
