<?php
	...
	$sql = "IF NOT EXISTS(select * from sp_itemmaker where item_no = '$item_no' and supplier_code = '$supplier_code') BEGIN 
	        insert into sp_itemmaker (create_date,
									operation_date,
									item_no,
									line_no,
									alter_procedure,
									...
									trial_sample_price 
									  ...
									  getdate(),
									  '$item_no',
									  (select isnull(max(line_no),0)  + 1 from sp_itemmaker where item_no = '$item_no'),
									  (select isnull(max(alter_procedure),0)  + 1 from sp_itemmaker where item_no = '$item_no'),
									  '$supplier_code',
									  0,
									  0,
									  $leadtime,
