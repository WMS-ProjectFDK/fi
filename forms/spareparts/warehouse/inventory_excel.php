...
$results = sqlsrv_query($connect, strtoupper($sql));
    while ($l = sqlsrv_fetch_array($results, SQLSRV_FETCH_ASSOC)) {
        ...
        if( $l['RECEIVE1'] > 0 || $l['OTHER_RECEIVE1'] > 0 || $l['ISSUE1'] > 0 || $l['OTHER_ISSUE1'] > 0){
            $sql2 = "
            select '[DETAIL]',t.item_no,cast(t.slip_date as varchar(11))  slip_date,slip_no, t.slip_type,
            ...
            $out .= "[DETAIL],ITEM_NO,SLIP DATE,SLIP_NO, SLIP_TYPE, RECEIVE, OTHER RECEIVE, ISSUE,OTHER_ISSUE" ;
            $out .= "\n";
  ...
  header("Content-Disposition: attachment; filename=inventory_report_detail.csv");
  echo $out;
?>