...
$query = "select ID,wo_no,st.lower_item_no as item_no, plt_no,
    CEILING((round((quantity * qty_prod) / quantity_base,0) + 
    Ceiling((quantity * qty_prod) / quantity_base  * failure_rate/100)) / isnull(bundle_qty,1)) * isnull(bundle_qty,1) 
    as QTY_REQ, st.level_no
    ...
    inner join (select ID, wo_no, plt_no, aa.item_no, qty_prod from ztb_plan_m aa
                 inner join mps_header bb on aa.wo_no = bb.work_order
    ) r on st.upper_item_no = r.item_no
    ...
    left outer join ztb_safety_stock c on st.lower_item_no = c.item_no and year = 'MSTR'
    where id = '$id'
...
// select ID,wo_no,st.lower_item_no as item_no, plt_no,
//     CEILING((round((quantity * qty_prod) / quantity_base,0) + 
//     Ceil((quantity * qty_prod) / quantity_base  * failure_rate/100)) / isnull(bundle_qty,1)) * isnull(bundle_qty,1) 
//     as QTY_REQ
//     from structure st
//     inner join  (select ID, wo_no, plt_no, aa.item_no, qty_prod,isnull(bb.bom_level,0) as bom_level from ztb_plan_m aa
...
//     left join (select item_no,sum(qty) qty from ztb_item_book group by item_no) zz on i.item_no = zz.item_no
//     left outer join ztb_safety_stock c on st.lower_item_no = c.item_no and year = 'MSTR'
//     where st.level_no = isnull(r.bom_level,0) and id = '$id'
//     and st.lower_item_no = i.item_no 
//     and lower_item_no < '9999999' 
//     and lower_item_no > 2000000
//     and lower_item_no not in ('2116095' ,'2116102' ,'2126023' ,'2126036' ,'2126040' ,'2126041' ,'2126052' ,'2145001' ,'2145008' ,'2150002'
...
$result = sqlsrv_query($connect, strtoupper($query));
