...
if (isset($_SESSION['id_wms'])){
	...
	foreach($queries as $query){
		...
		$cek ="select cast(slip_date as varchar(10)) as SLIP_DATE from mte_header where slip_no='$do_slip'";
		...
		if($dt->SLIP_DATE != $do_date){
			$upd = "update mte_header set slip_date = '$do_date', person_code='$user' where slip_no='$do_slip'";
		...
		if($do_lins==''){
			...
			$ins2 = "insert into mte_details ($field_dtl) 
				select top 1 $value_dtl from mte_details where not exists (select * from mte_details where slip_no ='".trim($do_slip)."' AND item_no = ".$do_item.")";
