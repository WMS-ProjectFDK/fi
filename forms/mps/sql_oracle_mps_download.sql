...
select  upper(to_char(sysdate,'dd/mon/yyyy hh24:mi')) as SYS_DATE, main.FLG, main.UPLOAD_DATE, main.ITEM_NO, i.DESCRIPTION as ITEM_NAME, 
c.CLASS_1 || c.CLASS_2 as BATERY_TYPE, i.GRADE_CODE as CELL_GRADE, main.PO_NO, main.PO_LINE_NO, 
case when main.STATUS = 'FM' then case when main.FLG = 'MPS' then  nvl(main.WORK_ORDER, main.PO_NO || '-' || c.CLASS_1 || c.CLASS_2 || i.GRADE_CODE || '-' || main.PO_LINE_NO) 
else  main.PO_NO || '-' || c.CLASS_1 || c.CLASS_2 || i.GRADE_CODE || '-' || main.PO_LINE_NO end else null end as WORK_ORDER, 
case when main.FLG = 'MPS' then nvl(main.CONSIGNEE, main.CONSIGNEE_SO) else main.CONSIGNEE end as CONSIGNEE, p.PACKING_TYPE_COMMENT as PACKAGE_TYPE, 
case when main.FLG = 'MPS' then nvl(main.DATE_CODE, main.DATE_CODE_SO) else main.DATE_CODE end as DATE_CODE, 
main.CR_DATE, main.REQUESTED_ETD, main.STATUS, s.LOWER_ITEM_NO as LABEL_ITEM_NUMBER, s.DESCRIPTION as LABEL_NAME, 
main.QTY, i.MAN_POWER, i.OPERATION_TIME, l.LABEL_TYPE_NAME as LABEL_TYPE, i.CAPACITY, main.REMARK 
from (
--------------------------------------------------------------------------------
  select 'MPS' as FLG, to_char(m_h.UPLOAD_DATE,'yyyymmdd hh24miss') as UPLOAD_DATE, m_h.ITEM_NO, m_h.PO_NO, 
  ...
  case when s.DATE_CODE is null then 
      GET_DATE_CODE(s.REQUESTED_ETD, s.ITEM_NO) 
  else
      s.DATE_CODE
  end as DATE_CODE_SO 
  from MPS_HEADER m_h, 
  (select distinct PO_NO, PO_LINE_NO from MPS_DETAILS where  trunc(MPS_DATE) >= trunc(add_months(trunc(sysdate), -1),'mm')) m_d,
  (select distinct s_h.CUSTOMER_PO_NO as PO_NO, s_d.CUSTOMER_PO_LINE_NO as PO_LINE_NO, s_h.CONSIGNEE_FROM_JP, s_d.DATE_CODE, s_d.ITEM_NO, s_d.ETD as REQUESTED_ETD
...
--------------------------------------------------------------------------------  
  union all
--------------------------------------------------------------------------------
  select 'SO' as FLG, null as UPLOAD_DATE, s.ITEM_NO, s.PO_NO, s.PO_LINE_NO, '' as WORK_ORDER, s.CONSIGNEE_FROM_JP as CONSIGNEE, 
  case when s.DATE_CODE is null then
    GET_DATE_CODE(s.REQUESTED_ETD, s.ITEM_NO) 
  else
    s.DATE_CODE
  end as DATE_CODE, null as CR_DATE, s.REQUESTED_ETD, 'FM' as STATUS, s.QTY, '' as REMARK, '' as CONSIGNEE_SO, '' as DATE_CODE_SO 
  from (
    select s_d.ITEM_NO, s_h.CUSTOMER_PO_NO as PO_NO, decode(trim(translate(s_d.CUSTOMER_PO_LINE_NO, '0123456789', '         ')), 
    null, to_char(to_number(s_d.CUSTOMER_PO_LINE_NO)), s_d.CUSTOMER_PO_LINE_NO) as PO_LINE_NO, s_h.CONSIGNEE_FROM_JP, s_d.DATE_CODE as DATE_CODE, 
  ...
  (select m_h.PO_NO, decode(trim(translate(m_h.PO_LINE_NO, '0123456789', '         ')), null, to_char(to_number(m_h.PO_LINE_NO)),  
  m_h.PO_LINE_NO) as PO_LINE_NO 
  from MPS_HEADER m_h, 
  (select distinct PO_NO, PO_LINE_NO from   MPS_DETAILS where  trunc(MPS_DATE) >= trunc(add_months(trunc(sysdate), -1),'mm')) m_d 
  where  m_h.PO_NO = m_d.PO_NO 
  and  m_h.PO_LINE_NO = m_d.PO_LINE_NO 
...
ITEM i, 
CLASS c, 
(select s1.UPPER_ITEM_NO, s1.LOWER_ITEM_NO, s2.DESCRIPTION 
...
and   substrb(UPPER(main.PO_NO),1,2) <>'SR'
