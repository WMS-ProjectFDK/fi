...
declare @nextMonth int = month(dateadd(m,0,getdate()))
declare @next2Month int = month(dateadd(m,1,getdate()))
...
from (select distinct item_no from ztb_safety_stock_1 where period in ( @nextMonth,@next2Month)) zx 
left outer join (select qty ,item_no from ztb_safety_stock_1 where period =  @nextMonth) z on zx.item_no = z.item_no 
left outer join item i on zx.item_no = i.item_no left outer join whinventory w on zx.item_no = w.item_no
left outer join (select qty qty_2,item_no from ztb_safety_stock_1 where period = @next2Month) z2 on zx.item_no = z2.item_no";
...
header('Content-Disposition: attachment; filename="SAFETY_STOCK.xls"');
