...
$data = file_get_contents('bc_return_pglosas_result_'.$user.'.json');
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D1', 'SLIP DATE')
            ->setCellValue('E1', 'SLIP NO.')
...
foreach ($someArray as $key => $data) {
    ...
    if ($data['BC_DOC'] != ''){
        $bc_format = number_format(trim($data['BC_DOC']),1);    
    }else{
        $bc_format = trim($data['BC_DOC']);
    }
    ...
    $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('D'.$no, $data['SLIP_DATE'])
                ->setCellValue('E'.$no, $data['SLIP_NO'])
                ->setCellValue('F'.$no, $data['CUSTOMER'])
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('BC_RETURN_WMS');
...
header('Content-Disposition: attachment; filename="BC_RETURN_WMS.xls"');
