...
$data = file_get_contents('bc_gr_spareparts_result_'.$user.'.json');
...
foreach ($someArray as $key => $data) {
    ...
    if ($data['BC_DOC'] != '' OR ! is_null($data['BC_DOC'])) {
        $bc_format = number_format($data['BC_DOC'],1);    
    }else{
        $bc_format = $data['BC_DOC'];
    }
    ...
    $objPHPExcel->getActiveSheet()->getStyle('F'.$no)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_TEXT);
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('BC_RECEIVE_SPAREPARTS');
...
header('Content-Disposition: attachment; filename="BC_RECEIVE_SPAREPARTS.xls"');
